  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <div className="font-lora font-bold text-gray-900">{sample.sample_label || `Sample ${sample.sample_id}`}</div>
          <div className="font-mont text-xs text-gray-400 mt-0.5">
            ...
            {sample.depth_cm && <span className="ml-2">Depth: {sample.depth_cm} cm</span>}
            {sample.latitude && <span className="ml-2">{sample.latitude.toFixed(4)}, {sample.longitude?.toFixed(4)}</span>}
          </div>
        </div>
        {confirming ? (
          <div className="flex gap-2">
            <button onClick={() => onDelete(sample.sample_id)} className="text-xs px-2 py-1 bg-red-500 text-white rounded font-mont">Confirm</button>
...
function AddSampleForm({ fieldId, onSaved, onCancel }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
      <h3 className="font-lora font-bold text-gray-900">Add Soil Sample</h3>
      {error && <div className="text-xs text-red-600 bg-red-50 rounded px-3 py-2 font-mont">{error}</div>}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="flex flex-col gap-1 col-span-2">
          <label className="text-xs font-semibold font-mont text-gray-500">Sample Label</label>
          <input value={form.sample_label} onChange={e => set('sample_label', e.target.value)} placeholder="e.g. North Corner"
            className="border border-gray-300 rounded-lg text-sm font-mont px-3 py-2" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold font-mont text-gray-500">Sample Date</label>
          <input type="date" value={form.sample_date} onChange={e => set('sample_date', e.target.value)}
            className="border border-gray-300 rounded-lg text-sm font-mont px-3 py-2" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold font-mont text-gray-500">Depth (cm)</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {numField('ph','pH')}
        {numField('organic_matter','Organic Matter %')}
        {numField('nitrogen','Nitrogen (kg/ha)')}
        {numField('phosphorus','Phosphorus (kg/ha)')}
        {numField('potassium','Potassium (kg/ha)')}
        {numField('sulfur','Sulfur (kg/ha)')}
        {numField('calcium','Calcium (kg/ha)')}
        {numField('magnesium','Magnesium (kg/ha)')}
        {numField('cec','CEC (meq/100g)')}
...
export default function PrecisionAgSoilSamples() {
  ...
  return (
    <AccountLayout Business={Business} BusinessID={BusinessID} PeopleID={localStorage.getItem('people_id')}
      pageTitle="Soil Samples" breadcrumbs={[{ label:'Dashboard', to:'/dashboard' }, { label:'Precision Ag' }, { label:'Soil Samples' }]}>
      <div className="max-w-full mx-auto space-y-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-lora text-2xl font-bold text-gray-900 mb-1">Soil Samples</h1>
            <p className="font-mont text-sm text-gray-500">Track pH, nutrients, and organic matter per field. Import from a CSV or enter manually.</p>
          </div>
          <div className="flex gap-2">
        ...
        {importResult && (
          <div className={`rounded-xl border px-4 py-3 font-mont text-sm ${importResult.imported > 0 ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
            Imported {importResult.imported} sample{importResult.imported !== 1 ? 's' : ''}.
            {importResult.errors?.length > 0 && <span className="ml-2">Errors: {importResult.errors.join(', ')}</span>}
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex gap-4 flex-wrap items-end">
          ...
          <div className="font-mont text-xs text-gray-400 text-xs self-end pb-2">
            CSV columns: sample_label, sample_date, latitude, longitude, depth_cm, ph, organic_matter, nitrogen, phosphorus, potassium, sulfur, calcium, magnesium, cec, notes
          </div>
        </div>
        ...
        {samples.length > 1 && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="font-mont text-sm font-semibold text-gray-600 mb-3">Field Average ({samples.length} samples)</div>
            ...
            <div className="mt-3 flex gap-4 font-mont text-xs text-gray-400">
              <span><span className="inline-block w-2.5 h-2.5 rounded-full bg-orange-400 mr-1" />Below optimal</span>
              <span><span className="inline-block w-2.5 h-2.5 rounded-full bg-green-400 mr-1" />Optimal</span>
              <span><span className="inline-block w-2.5 h-2.5 rounded-full bg-blue-400 mr-1" />Above optimal</span>
            </div>
          </div>
        )}
