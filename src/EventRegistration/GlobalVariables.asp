...
<% 
 ...
 'response.write("cookie LoggedIn=" & Request.Cookies("LoggedIn")) 
 'response.Write("session logged in=" & Session("LoggedIn"))
   
  
if len(Request.Cookies("PeopleFirstName") ) > 1 then
    Session("PeopleFirstName") = Request.Cookies("PeopleFirstName") 
end if
if len(Request.Cookies("PeopleID")) > 1 then
...
if Request.Cookies("LoggedIn")  = "True" or Session("LoggedIn") = "True" then
    Session("LoggedIn") = Request.Cookies("LoggedIn")
    LoggedIn = True
end if
'response.Write("LoggedIn=" & LoggedIn)
...
WebSiteName = "Livestock Of America Events"
...
'**************************************************************
sql = "select * from EventSiteDesign "
