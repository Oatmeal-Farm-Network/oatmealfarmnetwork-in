...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			StreamRequest.CopyTo streamFile, fileItem.Height
			Randomize
			fileItem.FileName = Session("custID") &  CInt((Day(now) + Month(now) + Year(now))) & "X" & fileItem.Height & int(rnd() * 12 ) & fileItem.FileName
	...
	Public Sub DumpData() 'only works if files are plain text
		...
		For Each f In UploadedFiles.Items
			...
			response.write "height: " & f.Height & "<br>"
		 Next
   	End Sub
