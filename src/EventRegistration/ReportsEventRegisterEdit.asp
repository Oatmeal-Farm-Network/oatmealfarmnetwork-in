...
<%  PageName = "Event Home" 
PageLink = "ReportsEventRegisterEdit.asp" %>
...
<a name="Top"></a>
<% PageTitleText = "Edit Registration"  %>
<!--#Include file="970Top.asp"-->
<br>
<table border = "0" width = "960">
<tr>
<td align = "center" valign = "top" class = "body">
To make changes to this registration change the quantities below and select the Submit Changes button at the bottom of the page.<br /><br />
<%
if len(CurrentPeopleID) < 1 then
    CurrentPeopleID = request.querystring("CurrentPeopleID")
end if 
if len(CurrentPeopleID) < 1 then
  CurrentPeopleID  = session("CurrentPeopleID")
  else
   session("CurrentPeopleID") = CurrentPeopleID 
 end if
CurrentPeopleID =  session("CurrentPeopleID") 
 StripeColor = "#F4F4F4" 
FirstGroup = True
		...
		sql2 = "SELECT * from Registration where Status='Paid' and PeopleID = " & CurrentPeopleID & " and Quantity > 0 and EventID=" & EventID 
	...
	<tr>
		  <td class = "body"  valign = "top"><b>Previously Ordered Services</b><br />
		  Below are services that you have previously ordered for this event:<br /><br />
		  ...
		  <tr><td colspan = "4" bgcolor = "#abacab"></td></tr>
		 <tr>
...
if request.querystring("RemoveHalterAnimal") = "True" then
  ...
  if len(RemoveHalterAnimalID)> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveHalterAnimalID & " and RegistrationType= 'Halter' "

	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If not rs.eof Then	

		Query =  "Delete * From RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveHalterAnimalID & " and RegistrationType= 'Halter' " 
...
if request.querystring("RemoveProductionAnimal") = "True" then
  ...
  if len(RemoveProductionAnimalID)> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveProductionAnimalID & " and RegistrationType= 'Production' "

	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If not rs.eof Then	

		Query =  "Delete * From RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveProductionAnimalID & " and RegistrationType= 'Production' " 
...
if request.querystring("RemoveCompanionAnimal") = "True" then
  ...
  if len(RemoveCompanionAnimalID)> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveCompanionAnimalID & " and RegistrationType= 'Companion' "

	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If not rs.eof Then	

		Query =  "Delete * From RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveCompanionAnimalID & " and RegistrationType= 'Companion' " 
...
if request.querystring("RemoveFleeceAnimal") = "True" then
  ...
  if len(RemoveFleeceAnimalID)> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveFleeceAnimalID & " and RegistrationType= 'Fleece' "
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If not rs.eof Then	

		Query =  "Delete * From RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveFleeceAnimalID & " and RegistrationType= 'Fleece' " 
...
if request.querystring("RemoveSpinOffAnimal") = "True" then
  ...
  if len(RemoveSpinOffAnimalID)> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveSpinOffAnimalID & " and RegistrationType= 'SpinOff' "
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If not rs.eof Then	
		Query =  "Delete * From RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & RemoveSpinOffAnimalID & " and RegistrationType= 'SpinOff' " 
...
sql2 = "select * from Animals where PeopleID = " & CurrentPeopleID & " order by FullName "
	acounter = 1
	Set rs2 = Server.CreateObject("ADODB.Recordset")
	rs2.Open sql2, conn, 3, 3 
...
<form action= "ReportsEventRegisterEdit.asp?EventID=<%=EventID%>&Update=True#FormTop" name="OrderForm" method = "post">


<%
sql = "select RegistrationID from RegistrationTemp where (Status='Paid' or Status = 'Draft' or Status='Update')   and EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and Quantity > 0 Order by RegistrationID Desc"
...
sql = "select * from RegistrationTemp where  EventID = " & EventID & " and PeopleID=" & CurrentPeopleID  
 rs.Open sql, conn, 3, 3   
while Not rs.eof  
    RegistrationID=rs("RegistrationID")
    'response.write("RegistrationID=" & RegistrationID )


 sql2 = "select * from RegistrationNotes where  RegistrationID = " & RegistrationID  
rs2.Open sql2, conn, 3, 3  
if not rs2.eof then
 VendorNotes = VendorNotes & " " & rs2("VendorNotes")
 HalterNotes = HalterNotes & " " & rs2("HalterNotes")
end if
rs2.close
...
<tr>
  <td class = "body" width= "380"><img src = "images/px.gif" width = "5" height = "1" alt = "Event registration"><h3>Service</h3></td>
  <td class = "body" align = "center" width = "110"><h3>Price</h3></td>
  <td class = "body" align = "center" width = "100"><h3>Quantity</h3></td>
  <td class = "body" align = "right" width = "100"><h3>Total</h3></td>
</tr>
</table>
</td>
</tr>
<tr>
  <td background = "images/Registrationbackground.jpg"><a Name= "FormTop"></a>
  	<table  border="0"  cellspacing="0" cellpadding="0" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" width = "690" align = "center">
...
if NOT(Update = "True") then 
	...
	sql = "select * from RegistrationTemp where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " Order by ItemPrice"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3
...
if Update = "True" then 
	...
	sql = "select RegistrationID from RegistrationTemp where (Status='Paid' or Status = 'Draft' or Status='Update')  and EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and ServiceDescription = '" & UpdateServiceDescription & "' Order by RegistrationID Desc"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If rs.eof Then	
...
while cint(counterx) < cint(AnimalQTY)
  ...
  if len(AnimalIDArray(counterx))> 0  then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & AnimalIDArray(counterx) & " and Registrationtype = 'Halter'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If rs.eof Then
	Query =  "INSERT INTO RegisteredAnimals (PeopleID, EventID, RegistrationType, AnimalRegistrationID ) " 
	Query =  Query & " Values ( " &  CurrentPeopleID & " ,"
	Query =  Query & " " &  EventID & " ,"
	Query =  Query & " 'Halter' ,"
...
while cint(counterx) < cint(ProductionQTY)
  ...
  if len(ProductionAnimalIDArray(counterx))> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & ProductionAnimalIDArray(counterx) & " and Registrationtype = 'Production'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If rs.eof Then
...
while cint(counterx) < cint(UnshownQTY)
  ...
  if len(CompanionAnimalIDArray(counterx))> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & CompanionAnimalIDArray(counterx) & " and Registrationtype = 'Companion'"
...
while cint(counterx) < cint(FleecesQTY)
  ...
  if len(FleeceAnimalIDArray(counterx))> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & FleeceAnimalIDArray(counterx) & " and Registrationtype = 'Fleece'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	If rs.eof Then
...
while cint(counterx) < cint(SpinOffQTY)
  ...
  if len(SpinOffAnimalIDArray(counterx))> 0 then
    sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and AnimalRegistrationID = " & SpinOffAnimalIDArray(counterx) & " and Registrationtype = 'SpinOff'"
...
Wend
 ...
 if ShowAdvertising = True then 
  sql = "select * from AdvertisingLevels  where AvaliableByItself = True and EventID = " & EventID 
	...
	sql2 = "select RegistrationID from RegistrationTemp where (Status='Paid' or Status = 'Draft' or Status='Update')  and EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and ServiceDescription = '" & UpdateServiceDescription & "' Order by RegistrationID Desc"
...
OrderExists = False
numorders = 0
sql = "select RegistrationID from RegistrationTemp where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID 
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	if not rs.eof then
		OrderExists = True
...
End If 
	  ...
	  if len(SponsorshipLevelQTYAvailable) > 0 then
        sqlqty = "select * from RegistrationTemp where EventID = " & EventID & " and ServiceDescription = 'Sponsorship - " & SponsorshipLevelName & "' "
...
counterx = 0 
sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and RegistrationType = 'Halter'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	while not rs.eof 
...
<%
 ...
 sqlp = "select * from AnimalHalterClassesLookup where SpeciesID = 1"
      ...
      <% if len(AnimalIDArray(counterx))> 0 then %>
         ...
         <a href = "EventAddAnimal.asp?AddAnimal=False&EditAnimal=True&AnimalAdded=True&AnimalID=<%=AnimalIDArray(counterx)%>" class = "body">Edit</a> | <a href = "ReportsEventRegisterEdit.asp?RemoveHalterAnimal=True&AnimalID=<%=AnimalIDArray(counterx)%>#Halter" class = "body">Remove</a>
      <% end if %>
	  </td>
	 </tr>
	<% 
...
counterx = 0 
sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and RegistrationType = 'Production'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	while not rs.eof 
  ...
  <select size="1" name="ProductionAnimalIDArray(<%=counterx %>)" width = "300" style="width: 300px" onchange="submit();">
    ...
    sqlp = "select * from AnimalProductionClassesLookup where SpeciesID = 1"
      ...
      <td class = "body" align = "center" width = "140">
      <% if len(ProductionAnimalIDArray(counterx))> 0 then %>
         <a href = "EventAddAnimal.asp?AddAnimal=False&EditAnimal=True&AnimalAdded=True&AnimalID=<%=ProductionAnimalIDArray(counterx)%>" class = "body">Edit</a> | <a href = "ReportsEventRegisterEdit.asp?RemoveProductionAnimal=True&AnimalID=<%=ProductionAnimalIDArray(counterx)%>#Production" class = "body">Remove</a>
      <% end if %>
	  </td>
	 </tr>
	<% 
...
counterx = 0 
sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and RegistrationType = 'Companion'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	while not rs.eof 
  ...
  <select size="1" name="CompanionAnimalIDArray(<%=counterx %>)"  width = "300" style="width: 300px" onchange="submit();">
      ...
      <td class = "body" align = "center" width = "140">
      <% if len(CompanionAnimalIDArray(counterx))> 0 then %>
         <a href = "EventAddAnimal.asp?AddAnimal=False&EditAnimal=True&AnimalAdded=True&AnimalID=<%=CompanionAnimalIDArray(counterx)%>" class = "body">Edit</a> | <a href = "ReportsEventRegisterEdit.asp?RemoveCompanionAnimal=True&AnimalID=<%=CompanionAnimalIDArray(counterx)%>#Companion" class = "body">Remove</a>
      <% end if %>
	  </td>
	 </tr>
	<% 
...
counterx = 0 
sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and RegistrationType = 'Fleece'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	while not rs.eof 
	 counterx = counterx +1
  ...
  <select size="1" name="FleeceAnimalIDArray(<%=counterx %>)" width = "300" style="width: 300px"  onchange="submit();">
      ...
      <td class = "body" align = "center" width = "140">
      <% if len(FleeceAnimalIDArray(counterx))> 0 then %>
         <a href = "EventAddAnimal.asp?AddAnimal=False&EditAnimal=True&AnimalAdded=True&AnimalID=<%=FleeceAnimalIDArray(counterx)%>" class = "body">Edit</a> | <a href = "ReportsEventRegisterEdit.asp?RemoveFleeceAnimal=True&AnimalID=<%=FleeceAnimalIDArray(counterx)%>#Fleece" class = "body">Remove</a>
      <% end if %>
	  </td>
	 </tr>
	<% 
...
counterx = 0 
sql = "select * from RegisteredAnimals where EventId = " & EventID & " and PeopleID =" & CurrentPeopleID & " and RegistrationType = 'SpinOff'"
	Set rs = Server.CreateObject("ADODB.Recordset")
	rs.Open sql, conn, 3, 3   
	while not rs.eof 
  ...
  <select size="1" name="SpinOffAnimalIDArray(<%=counterx %>)"  width = "300" style="width: 300px" onchange="submit();">
      ...
      <td class = "body" align = "center" width = "140">
      <% if len(SpinOffAnimalIDArray(counterx))> 0 then %>
         <a href = "EventAddAnimal.asp?AddAnimal=False&EditAnimal=True&AnimalAdded=True&AnimalID=<%=SpinOffAnimalIDArray(counterx)%>" class = "body">Edit</a> | <a href = "ReportsEventRegisterEdit.asp?RemoveSpinOffAnimal=True&AnimalID=<%=SpinOffAnimalIDArray(counterx)%>#SpinOff" class = "body">Remove</a>
      <% end if %>
	  </td>
	 </tr>
	<% 
...
End If 
	  if len(VendorStallQTYAvailable) > 0 then
        sqlqty = "select * from RegistrationTemp where PeopleID = " & CurrentPeopleID & " and  EventID = " & EventID & " and ServiceDescription = 'Vendor - " & VendorStallName & "' order by ItemPrice Desc"
         Set rsqty = Server.CreateObject("ADODB.Recordset")
         rsqty.Open sqlqty, conn, 3, 3   
	     while not rsqty.eof 
...
<tr><td class = "body" colspan = "4" valign = "top" align = "center">
<textarea name="VendorNotes" cols="80" rows="2" wrap="VIRTUAL" class = "body" onchange="submit();"><%= VendorNotes%></textarea>
</td></tr>
...
End If 
 ...
 'response.Write("ClassInfoMaximumStudents=" & ClassInfoMaximumStudents & "<br>")
	  if len(ClassInfoMaximumStudents) > 0 then
        sqlqty = "select * from RegistrationTemp where EventID = " & EventID & " and ServiceDescription = 'Classes - " & ClassInfoTitle & "' "
         Set rsqty = Server.CreateObject("ADODB.Recordset")
         rsqty.Open sqlqty, conn, 3, 3   
	     while not rsqty.eof 
	     ' response.Write("ClassInfoMaximumStudents=" & ClassInfoMaximumStudents & "<br>")
...
<% If FirstGroup = True then
 ...
 <% end if  %>  
	...
	<tr ><td class = "body" colspan = "3" height = "30" align = "right" ><h3>Total:</h3><img src = "/images/px.gif" width = "5" alt = "<%=EventName%> Event Registration"></td>
		<td class = "body" colspan = "1" height = "30" align = "right"><h3><%=formatcurrency(OrderTotal,2) %><img src = "/images/px.gif" width = "10" alt = "<%=EventName%> Event Registration"></h3>
...
<% 
sqlq = "select * from ordersSetupEvents where Payment_status='Completed' and EventID = " & EventID & " and PeopleID=" & CurrentPeopleID & " order by DateAdded Desc"
...
<tr><td  class = "body" colspan = "2"><h2>Payments</h2>
Below are the payments that have been made:
</td></tr>
<tr><td class = "body" width = "150" align = "center"><b>Date</b></td>
<td class = "body" width = "150" align = "center"><b>Payment Type</b></td>
<td class = "body" width = "100" align = "center"><b>Amount</b></td></tr>
<% 
TotalPayments = 0
while Not rsq.eof  

  
 OrderEventID=rsq("OrderEventID")
OrderPeopleID=rsq("PeopleID")
Payment_status=rsq("Payment_status")
Payment_amount=rsq("Payment_amount")
Payment_currency=rsq("Payment_currency")
Payer_email=rsq("Payer_email")
first_name=rsq("first_name")
Payment_Type=rsq("Payment_Type")
last_name=rsq("last_name")
DateAdded = rsq("DateAdded") 
if len(Payment_amount) > 0 then
TotalPayments = TotalPayments + cint(Payment_amount)
end if
%>
<Tr>
<td class = "body"  align = "center">
 <%= formatdatetime(DateAdded,2) %>
...
<td class = "body" align = "right">
<% if len(Payment_amount) > 0 then %>
<%= formatcurrency(Payment_amount,2) %><img src = "images/px.gif" width = "5" height = "1" />
<% end if %>
</td></Tr>
...
rsq.close
%>
<tr><td class = "body" align = "right" colspan = "2"><b>Total Amount Paid:</b></td>
<td class = "body" align = "right" > <b><%=formatcurrency(TotalPayments,2) %></b><img src = "images/px.gif" width = "5" height = "1" />
</td></tr>
<tr><td class = "body" align = "right" colspan = "2"><b>Amount Due:</b></td>
<td class = "body" align = "right" > <b><%=formatcurrency(OrderTotal - TotalPayments,2)  %></b><img src = "images/px.gif" width = "5" height = "1" />
</td></tr>
</table>
<% end if %>
</td></tr>
<tr ><td class = "body" colspan = "4" height = "30" align = "right" >	

</form>
		<form action= "ReportsEventRegisterEditCompletion.asp?CurrentEventID=<%=EventID%>&CurrentPeopleID=<%=CurrentPeopleID%>&OrderTotal=<%=OrderTotal%>" name="OrderForm" method = "post">


<b>Amount Paid:</b><img src = "/images/px.gif" width = "5" alt = "<%=EventName%> Event Registration">$<input type = "text" class="positive" name="PaymentAmount" size ="5"/>
	...
	<tr ><td class = "body" colspan = "4" height = "30" align = "right" ><b>Payment Type:</b><img src = "/images/px.gif" width = "5" alt = "<%=EventName%> Event Registration">	
	
	
	<INPUT TYPE=RADIO NAME="PaymentType" VALUE="Check">Check
<INPUT TYPE=RADIO NAME="PaymentType" VALUE="Cash">Cash
<INPUT TYPE=RADIO NAME="PaymentType" VALUE="Online Payment">Online Payment
...
<table  border="0"  cellspacing="0" cellpadding="0" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" width = "700" align = "center">	
<tr>
  <td class = "body" align = "right" background = "images/Registrationheader.jpg" height = "39">
...
dontshow = false
 ...
 if OrderTotal < 1  or dontshow = true then %>
 <input type=submit value="Submit Changes" disabled="disabled" class = "regsubmit2" >
<% else %>
