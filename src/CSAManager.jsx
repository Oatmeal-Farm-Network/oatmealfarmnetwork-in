...
export function CSABrowse() {
  ...
  return (
    <div style={{ backgroundColor: '#f7f2e8', minHeight: '100vh' }}>
      <PageMeta title="FPO Shares — Oatmeal Farm Network" description="Find Farmer Producer Organisation produce shares near you." />
      <Header />
      <div style={{ background: 'linear-gradient(90deg,rgba(255,255,255,0.93) 0%,rgba(255,255,255,0) 100%)', borderBottom: '1px solid #e5e7eb' }}>
        <div className="max-w-5xl mx-auto px-6 py-10">
          <Breadcrumbs items={[{ label: 'FPO Shares' }]} />
          <h1 className="text-3xl font-bold text-gray-900 mt-1" style={{ fontFamily: "'Lora','Times New Roman',serif" }}>FPO Shares</h1>
          <p className="text-gray-500 text-sm mt-1">Farmer Producer Organisation produce shares — subscribe to a local farm or FPO share.</p>
        </div>
      </div>
      <div className="max-w-5xl mx-auto px-6 py-6">
        {loading ? <p className="text-gray-400">Loading…</p> : plans.length === 0 ? (
          <div className="text-center py-20 text-gray-400">No FPO shares available right now.</div>
        ) : (
            ...
            {plans.map(p => (
              <button key={p.PlanID} onClick={() => setSelected(p)}
                ...
                <div className="flex flex-wrap gap-3 mt-3 text-sm">
                  {p.PricePerShare && <span className="font-bold text-gray-800">{formatInr(p.PricePerShare)}/{p.Frequency?.toLowerCase() || 'share'}</span>}
          ...
          <div className="bg-white w-full max-w-lg h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {selected.PricePerShare && <div><span className="font-semibold text-gray-600">Price:</span> {formatInr(selected.PricePerShare)}/{selected.Frequency?.toLowerCase()}</div>}
  ...
  return (
    <div style={{ backgroundColor: '#f7f2e8', minHeight: '100vh' }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <Breadcrumbs items={[{ label: 'FPO Shares', to: '/fpo' }, { label: 'Manage' }]} />
        <h1 className="text-2xl font-bold text-gray-900 mt-2 mb-6" style={{ fontFamily: "'Lora','Times New Roman',serif" }}>Manage FPO Plans</h1>
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <div className="font-bold text-gray-800 mb-4">{editing ? 'Edit Plan' : 'Create FPO Plan'}</div>
            ...
            <div key={p.PlanID} className="bg-white rounded-xl border border-gray-200 px-5 py-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">{p.Frequency} · {formatInr(p.PricePerShare)}/share · {p.ActiveSubs ?? 0} active subscribers{p.Capacity ? ` / ${p.Capacity} capacity` : ''}</div>
                </div>
