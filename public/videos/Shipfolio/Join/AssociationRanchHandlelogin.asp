...
if len(Email) > 5 and len(password) > 5 then
	sql2 = "select * from People where Upper(Peopleemail) = '" & Email & "' and (PeoplePassword = '" & password & "' or PeoplePassword2 = '" & password & "'  ) order by PeopleID Asc"
...
Response.Redirect("/members/Default.asp" )
      ...
      Set conn = Nothing
  Response.Redirect("Join/Default.asp?Fail=True" )
     end if  
%>
