...
if len(BusinessID) < 1 then
	Query =  "INSERT INTO Business (BusinessName, BusinessTypeID)" 
	Query =  Query & " Values ('" & BusinessName & "', " & BusinessTypeID & " )" 
...
If ExistingAccount = False then
		Query =  "INSERT INTO People (AddressID, ActivationCode, country_id, ReferringPeopleID, WebsitesID, Accesslevel, custAIStartService, custAIEndService, SubscriptionLevel, Owners, BusinessID,  PeopleFirstName, PeopleLastName, PeoplePhone, Peopleemail, PeopleCell, PeopleActive,  PeoplePassword , permissionCheckbox, LivestockLegalDisclaimer, SalesLegalDisclaimer, PeopleCreationDate  )" 
       ...
       If membership = "Intro" or len(membership)< 1 then
        ...
        Query =  Query & " '" & Password   & "', " 
	        Query =  Query & " '" & permissionCheckbox   & "', " 
	        Query =  Query & " '" & LivestockLegalDisclaimer   & "', " 
	        Query =  Query & " '" & SalesLegalDisclaimer   & "', " 
...
Query =  "INSERT INTO BusinessAccess (BusinessID, AccesslevelID)" 
Query =  Query & " Values ('" & BusinessID & "', 3 )" 
	'response.write("Query=" & Query )
Conn.Execute(Query) 
...
<div>
   <div>
		...
		<blockquote><h2>One last step.</h2>
			...
			Give that button, down below, a click to confirm that email address of yours. Soon as you do, an email's gonna wing its way to <b><%=PeopleEmail%></b>. Just follow along with the instructions in that there email.<br /><br />
			<center>

				<form action="SetupAccountStep4.asp?PeopleID=<%=PeopleID %>&ActivationCode=<%=ActivationCode %>&PeopleFirstName=<%=PeopleFirstName%>&PeopleEmail=<%=PeopleEmail%>" method="POST">
				  <button type="submit" class = "regsubmit2" style="min-width:90px">Send Email</button>
				</form><br />
				...
				<% showsms = false
					...
					<form action="SetupAccountStep4SMS.asp?PeopleID=<%=PeopleID %>" method="POST">
						<button type="submit" class ="regsubmit2" style="min-width:90px">Phone</button>
					</form>
				<% end if %>
			</center>
