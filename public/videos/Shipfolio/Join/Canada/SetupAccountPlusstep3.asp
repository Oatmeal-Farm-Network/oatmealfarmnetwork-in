...
confirmpassword =Request.form("RightShoe")
...
if MissingData = True or SpecialChecterFound = False or  len(StateIndex) < 1 or passwordmatch = False then
response.redirect("SetupAccountPlusStep2.asp" & variables & "&Stepback=True&StateIndexFound=" & StateIndexFound & "&EmailsMatch=" & EmailsMatch  & "&passwordmismatch=" & passwordmismatch & "&SpecialChecterFound=" & SpecialChecterFound & "&MissingPassword=" & MissingPassword)
end if
...
<div class="container d-flex justify-content-center" style = "max-width:460px">
  <div class="row">
  <div class="col">
...
'response.write("Password=" & Password & "!")
       ...
       If membership = "Intro" then
		...
		Query =  Query & " '" &  left(PeoplePhone, 25) & "', " 
		Query =  Query & " '" &  left(Peopleemail, 25) & "', " 
		Query =  Query & " '" &  left(Peoplecell, 25)  & "', " 
		Query =  Query & "'-1', " 
        Query =  Query & " '" & left(Password, 25)   & "', " 
...
strTo = PeopleEmail & ", contactus@livestockoftheworld.com"
...
strBody = strBody & "Account Confirmation<br>------------------------------------------------<br>" & vbCrLf
strBody = strBody & "Thank you for registering with us." & vbCrLf
strBody = strBody & "We require that you validate your registration to ensure that the email address you entered was correct. This protects against unwanted spam and malicious abuse.<br><br>To activate your account, simply click on the following link:" & vbCrLf
...
<input type="hidden" name="currency_code" value="<%=LocalCurrency %>">
    <input type="hidden" name="cancel_return" value="http://www.LivestockOfTheWorld.com/Join/USA/Default.asp?PeopleID=<%=PeopleID%>&Peopleemail=<%=Peopleemail %>">
...
<input type="hidden" name="bn" value="PP-SubscriptionsBF:btn_subscribeCC_LG.gif:NonHostedGuest">
   ...
   <input type="hidden" name="cbt" value="Return to Livestock Of The World">
    ...
    <input type="image" src="/logos/PaypalButton.png" height = 130px border="0" name="submit" >
</form>
</center>
</td></tr></table>
</div>
