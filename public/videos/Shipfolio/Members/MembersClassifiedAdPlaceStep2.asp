...
response.write("<br>prodCategory1ID = " & prodCategory1ID  )
response.write("<br>prodCategory2ID = " & prodCategory2ID )
response.write("<br>prodCategory3ID = " & prodCategory3ID )

response.write("<br>prodSubCategory1ID = " & prodSubCategory1ID )
response.write("<br>prodSubCategory2ID = " & prodSubCategory2ID )
response.write("<br>prodSubCategory3ID = " & prodSubCategory3ID)
...
variables = variables & "&ProdProductID=" & ProdProductID
end if
...
response.write("<br>Category1ID=" & Category1ID )
...
response.write("variables=" & variables )


if len(prodCategory2ID) > 1 then
if prodCategory2ID > 0 then
	variables = variables & "&prodCategory2ID=" & prodCategory2ID
end if
end if
if len(prodCategory3ID) > 1 then
if prodCategory3ID > 0 then
	variables = variables & "&prodCategory3ID=" & prodCategory3ID
end if
end if

if len(prodSubCategory1ID) > 1 then
if prodSubCategory1ID > 0 then
	variables = variables & "&prodSubCategory1ID=" & prodSubCategory1ID
end if
end if
if len(prodSubCategory2ID) > 1 then
if prodSubCategory2ID > 0 then
	variables = variables & "&prodSubCategory2ID=" & prodSubCategory2ID
end if
end if

if len(prodSubCategory3ID) > 1 then
	variables = variables & "&prodSubCategory3ID=" & prodSubCategory3ID
end if
	
if len(SubCategory3ID) > 1 then
...
if ProdForSale = False then
    ProdForSale = "0"
else
    ProdForSale = "1"
end if

if ProdQuantityAvailable = False then
    ProdQuantityAvailable = "0"
else
    ProdQuantityAvailable = "1"
end if

if prodCountryTaxIsActive = False then
    prodCountryTaxIsActive = "0"
else
    prodCountryTaxIsActive = "1"
end if

response.write("ProdCustomOrder=" & ProdCustomOrder )

if ProdCustomOrder = "False" then
    ProdCustomOrder = "0"
else
    ProdCustomOrder = "1"
end if  
...
response.write(sql2)
 ...
 session("ID") = ProdID
  session("ProdID") = ProdID
Query =  "INSERT INTO SFProducts ( PeopleID, ProductID, prodProductID, ProdName, ProdMadeIn, ProdCustomOrder , prodCountryTaxIsActive, ProdQuantityAvailable , "
...
if len(session("prodCategory3ID")) > 0 and len(session("SubCategory3ID")) > 0 then
Query =  " Insert into productCategoriesList (ProductID, prodCategoryId, prodSubCategoryId)"
Query =  Query & " Values ('" &  ProdID  & "'," & session("prodCategory3ID") & "," & session("prodSubCategory3ID") & ")"
Conn.Execute(Query) 
end if 

if len(session("prodCategory2ID")) > 0 and session("SubCategory2ID") > 0 then
Query =  " Insert into productCategoriesList (ProductID, prodCategoryId, prodSubCategoryId)"
Query =  Query & " Values (" &  ProdID  & "," & session("prodCategory2ID") & "," & session("prodSubCategory2ID") & ")"
Conn.Execute(Query) 
end if 

if len(session("prodCategory1ID")) > 0 then
Query =  " Insert into productCategoriesList (ProductID, prodCategoryId, prodSubCategoryId)"
Query =  Query & " Values ('" &  ProdID  & "'," & session("prodCategory1ID") & "," & session("prodSubCategory1ID") & ")"
Conn.Execute(Query) 
end if 
...
Conn.close
set conn=nothing 
response.Redirect("MembersProductAdded.asp?ProdId=" & ProdID & "&prodCategory1ID=" & Session("Category1ID") & "&prodCategory2ID=" & Session("Category2ID") & "&prodCategory3ID=" & Session("Category3ID") & "&prodSubCategory1ID=" &  Session("SubCategory1ID") & "&prodSubCategory2ID=" & Session("SubCategory2ID") & "&prodSubCategory3ID=" & Session("SubCategory2ID") )
%>
</Body>
</HTML>
