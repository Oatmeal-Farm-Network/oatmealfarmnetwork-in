...
sql="select distinct BusinessFacebook, BusinessX, BusinessInstagram, BusinessPinterest, BusinessTruthSocial, BusinessBlog, BusinessYouTube, BusinessOtherSocial1, BusinessOtherSocial2, PeopleShowAddress, PeopleShowMap, PeopleShowPhone, PeopleFirstName, PeopleMiddleInitial, PeopleLastName, PeoplePhone, PeopleCell, PeopleFax, PeopleEmail, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip, Business.WebsitesID, weblink, Businessname, Owners, People.PeopleID, People.logo, Business.Logo, custLastAccess,  Business.BusinessName from People, Business, address where People.accesslevel > 0 and People.BusinessID = Business.BusinessID and People.addressID = Address.AddressID and  BusinessTypeID = " & BusinessTypeID & "  and address.country_id = '" & Currentcountry_id  & "' " & statesort &  citysort & namesort & ownersort & " order by custLastAccess desc,  Business.BusinessName, People.Logo asc "
...
if ShowRanchlinks = True then
    ...
    RanchProfileLink = "/Farms/FarmListing.asp.asp?Peopleid=" & PeopleID 
else
    RanchLink = "/Farms/FarmListing.asp?CurrentPeopleID=" & PeopleID 
    RanchProfileLink = "/Farms/FarmListing.asp?Peopleid=" & PeopleID
    LinkText= "Contact Restaurant"
end if
