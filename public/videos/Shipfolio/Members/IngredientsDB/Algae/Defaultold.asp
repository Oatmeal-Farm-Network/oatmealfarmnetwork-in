...
<% Ingredienttype = "Algae" 
DirectoryName = "Algae"
IngredientCategoryID = 14
description = "Explore a comprehensive list of " & Ingredienttype & "Ingredient types and their varieties."

Dim IngredientID, IngredientName, IngredientDescription
IngredientID = Request.QueryString("IngredientID")

' Initialize variables for metadata
IngredientName = "Varietals" ' Default
IngredientDescription = "Explore various Ingredient varietals." ' Default

If IsEmpty(IngredientCategoryID) Or IsNull(IngredientCategoryID) Or IngredientCategoryID = "" Then
...
sqlIngredient = "SELECT IngredientName, IngredientDescription FROM Ingredients WHERE IngredientCategoryID = " & IngredientCategoryID 
...
%>
<title><%=WebSiteName %> | <%=IngredientName %> Varietals</title>
<meta name="title" content="<%=WebSiteName %> | <%=IngredientName %> Varietals"/>
<meta name="description" content="Explore all known varietals of <%=IngredientName %>, including their descriptions, growing requirements, and nutrient profiles. <%=IngredientDescription %>"/>
<meta name="keywords" content="<%=IngredientName %>, varietals, Ingredient varieties, <%=IngredientName %> growing, Ingredient database, food Ingredients, gardening, agriculture"/>
...
<div align="center">
    <div class="container">
        <div>
            <div class="body">
                <h1><%=IngredientName %> Varietals</h1><br />
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-12 body text-left">
        <p>Below is a list of all known varietals for <%=IngredientName %>. Click on a varietal name to view more detailed information.</p>
        <br />
    </div>
</div>
        ...
        <div class="varietal-table-container">
            <table class="varietal-table">
                ...
                <tbody>
                    ...
                    sqlVarietals = "SELECT PV.IngredientVarietyID, PV.IngredientVarietyName, PV.IngredientVarietyDescription, " & _
                                   ...
                                   "LEFT JOIN IngredientHardinessZoneLookup PHZ ON PV.ZoneID = PHZ.ZoneID " & _
                                   "LEFT JOIN HumidityLookup H ON PV.HumidityID = H.HumidityID " & _
                                   "LEFT JOIN NutrientLookup NL ON PV.IngredientNutrientID = NL.NutrientID " & _
                                   "WHERE PV.IngredientID = " & IngredientID & " " & _
                                   "ORDER BY PV.IngredientVarietyName"
                    ...
                    If Not rsVarietals.EOF Then
                        Do While Not rsVarietals.EOF
                            ...
                            pvID = rsVarietals("IngredientVarietyID")
                            pvName = rsVarietals("IngredientVarietyName")
                            pvDescription = rsVarietals("IngredientVarietyDescription")
                    ...
                    %>
                            <tr>
                                <td><a href="VarietalDetail.asp?IngredientVarietyID=<%=pvID %>"><%=pvName %></a></td>
                    ...
                    %>
                        <tr>
                            <td colspan="10" class="text-center">No varietals found for <%=IngredientName %> in the database.</td>
                        </tr>
                    <%
                    End If
                    rsVarietals.Close
