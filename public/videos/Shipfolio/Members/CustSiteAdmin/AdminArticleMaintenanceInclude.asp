...
<select size="1" name="CatID">
					<option name = "ALinkID0" value= "<%=ArticleCatID%>" selected><%=ArticleCatID%></option>
					<% count = 1
						While CatCounter < FinalCatCounter
						CatCounter= CatCounter +1
					%>
						<option name = "CatID" value="<%=CatID(CatCounter,0)%>">
	...
	<tr>
		<td  align = "center" valign = "middle" colspan = "2">
			<input type=submit value = "Add Link" size = "110" class = "Submit" >
			</form>
		</td>
</tr>
</table>
...
%>
	...
	<tr >
	 ...
	 <% If Len(LinkImageArray(rowcount)) < 2 Then
	         LinkImageArray(rowcount) = "ImageNotAvailable.jpg"
		End If %>

			<img src = "../../uploads/links/<%= LinkImageArray(rowcount)%>" width = "65"><br>
		          ...
		          <select size="1" name="CatID(<%=rowcount%>)">
					...
					%>
						<option value="<%=CatID(CatCounter,0)%>">
							<%=CategoryName(CatCounter,0)%><%=CatID(CatCounter,0)%>
						</option>
					<% 	count = count + 1
					wend %>
					</select>
		...
		<td valign = "top" >
			...
			<form action= 'AdminLinkDeleteHandleForm.asp' method = "post">
			   ...
			   <tr>
				...
				<td>
					<br>
					<input type=submit value = "Delete" size = "110" class = "Submit" >
				</td>
			  </tr>
		    </table>
		  </form>
