...
ProdProductID=trim(request.form("ProductID")) 
...
Conn.close
'set conn=nothing 
  response.Redirect("AdminClassifiedAdPlace.asp" & Missing & "&ProdNameFound=true" & variables)
end if
...
response.write(sql2)
 ...
 session("ID") = ProdID
  session("ProdID") = ProdID
Query =  "INSERT INTO SFProducts ( PeopleID, ProductID, ProdName, ProdMadeIn, ProdCustomOrder , prodCountryTaxIsActive, ProdQuantityAvailable , "
