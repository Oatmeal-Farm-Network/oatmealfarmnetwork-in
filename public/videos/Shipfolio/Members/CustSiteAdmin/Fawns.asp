...
<title><%= WebSiteName %>Fawn Alpacas for Sale</title>
...
<% PageTitle = "Fawn Alpacas for Sale" %>
...
<% 	conn = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
	...
	' Get marketing text for the top of the page:
		...
		sql = "SELECT distinct Animals.*, Pricing.*, Photos.Listpageimage, Ancestors.* FROM Animals, Pricing, Photos, Ancestors WHERE Animals.ID=Pricing.ID  And Animals.ID=Photos.ID And Animals.ID=Ancestors.ID and ForSale = Yes and (Colorcategory = 'Fawn' )  order by category, AValue " 
    ...
    If (Not rs.eof) And (Not Trim(UCase(rs("AValue"))) = "SOLD!") Then
		rs.close
		sql = "SELECT distinct Animals.*, Pricing.*, Photos.Listpageimage, Ancestors.* FROM Animals, Pricing, Photos, Ancestors WHERE Animals.ID=Pricing.ID  And Animals.ID=Photos.ID And Animals.ID=Ancestors.ID and ForSale = Yes and (Colorcategory = 'Fawn' ) order by Female, Price DESC" 
