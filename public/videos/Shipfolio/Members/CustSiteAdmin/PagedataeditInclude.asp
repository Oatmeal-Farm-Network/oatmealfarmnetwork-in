...
<% end if %>
  ...
  <table border = "0" cellspacing="0" cellpadding = "0" align = "center" width = "<%=screenwidth -40%>" ><tr><td class = "body" align = "left">
<%
'*******************************************************************************************
'WYSIWYG Scripts
'*******************************************************************************************
...
<script type="text/javascript">
    ...
    mysettings.ImagePopupWidth =400;
    mysettings.ImagePopupHeight = 245;
    WYSIWYG.attach("textarea1<%=Block%>", mysettings);
</script>
<form action= 'PageDataHandleForm2.asp?filename=<%=filename%>&PeopleID=<%=PeopleID%>' method = "post">
...
<td valign="top" width = "300">
					    ...
					    <div align = "left"><b>Images must be in .JPG, .JPEG, .GIF, or .PNG format<br /> and less than 300KB in size.</b></div><br> 
							...
							<h2>No Image</h2>
							<% End If %>
							<form  method="POST" enctype="multipart/form-data" action="PageDatauploadImage.asp?PageLayout2ID=<%=TempPageLayout2ID%>&filename=<%=filename%>&PeopleID=<%=PeopleID%>" >
								Upload Photo: <br>
								<input name="attach1" type="file" size=20 class = "regsubmit2">
