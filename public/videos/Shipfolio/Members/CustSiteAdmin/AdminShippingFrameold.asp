...
if len(ProdID)> 0 then
    response.Redirect("AdminShippingFrame.asp?ProdID=" & ProdID )
else
    response.Redirect("AdminShippingFrame.asp?ServicesID=" & ServicesID )
end if
end if
...
<tr><td class = "body" ><br /><b>Ship to</b></td><td class = "body" width = '75'><br /><center><b>Cost</b></center></td><td class = "body" width = '110'><center><b>With<br />Another Item</b><a class="tooltip" href="#"><b>?</b><span class="custom info">When you add a location, it will automatically be added to your list below:</span></a></center></td><td width = '9' class = "body"><center><b>Delete</b></center></td></tr>
<% 
if len(ProdId)> 0 then
sql = "select * from sfShipping where ProdID=" & ProdID
else
...
<% if len(ProdId)> 0 then %>
 ...
 <% end if %>
  <input type=submit value = "X" class = "regsubmit2"  <%=Disablebutton %> ><input name="shipID" value="<%=rs("shipID") %>" type = "hidden"></td>
  <td></td></tr></form>
