...
'response.write(ID)
   ...
   if ID= "" Then
			Response.Redirect("AlpacaSale.asp")
	end if
			...
			sql = "select FemaleData.id, Animals.id from FemaleData, Animals where animals.ID = FemaleData.ID and Animals.ID=" & ID
				gender = "female"
				'response.write(sql)
		...
		if not rs.eof then 
           ...
           If photoID = "nophoto" then 
				click = "<img width=""285"" src=""/Uploads/DetailPage/NotAvailable.jpg""> "
			Else
