...
<tr>
	<td align = "center"><br><br>
		...
		<table border="0" cellspacing="0" cellpadding="0" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" align = "center"  valign = "top" width = "<%=TextWidth%>">
		<tr>
			<td  valign = "top" background = "images/H1background.jpg"></td>
			<td class = "body"  valign = "top"><h1></h1></td>
			<td class = "body"  valign = "top"><h1></h1></td>
		</tr>
		</table>
...
sql = "select * from Testimonials where TestimonialsType = 'Business' order by CustomerName"
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3 
if rs.eof then %>
...
<tr>
	<td class = "body">
		currently we do not have any testimonials entered. Please check back later.
	</td>
</tr>
...
<tr>
	<td class = "body">
		...
		<% if len(rs("Name")) > 3 then %>
			- <%=rs("Name")%><br>
		<% end if %>
		<a href = "http://<%=rs("URL")%>" class = "body" target = "blank"><%=rs("CustomerName")%></a>
		<br><br><br><br>
		</div>
	</td>
</tr>
