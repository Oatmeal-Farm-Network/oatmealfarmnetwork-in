...
DamAri=Request.Form("DamCLAA") 
DamAri=Request.Form("DamBAS") 
...
DamDamBAS=Request.Form("DamDamBAS") 
...
DamDamDamBAS=Request.Form("DamDamDamBAS") 
...
%>
<!--#Include File="AdminCheckAncestorDataInclude.asp"-->
<%
Dam   = Ancestor
Dam2   = Ancestor2
...
If len(Dam2) > 0 then
 ...
 sql = "select animals.ID, ancestors.*, colors.* from animals, Ancestors, colors where animals.ID = Colors.ID and animals.ID = Ancestors.ID and FullName =  '" & Dam2 & "'"
 'response.Write("sql=" & sql)
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
 
 if len(Damcolor) > 0 then
 else
...
response.write("<br>SireColor0=" & SireColor & "<br>")
if len(trim(SireColor)) > 0 then
else
response.write("<br>SireColor1=" & SireColor & "<br>")
 If len(rs("Color1")) > 0 or len(rs("Color2")) > 0 or len(rs("Color3")) > 0 or len(rs("Color4")) > 0 or len(rs("Color5")) > 0 then
    SireColor = ""
     If len(rs("Color1")) > 0 then
...
response.write("<br>SireColor2=" & SireColor & "<br>")
    Sire = Sire2  
...
Conn.Close
set conn = nothing
if SubmitType = "Save & Proceed To Next Page" then
response.redirect("AdminAnimalAdd3.asp?id=" & id & "&Name=" & Name & "&speciesID=" & speciesID)
  else
  'response.write("speciesID=" & speciesID)
response.redirect("AdminAnimalAdd2.asp?id=" & id & "&Name=" & Name & "&speciesID=" & speciesID)
end if
%>
 </Body>
</HTML>
