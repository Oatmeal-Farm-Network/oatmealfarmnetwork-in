...
<title>Alpacas at Lone Ranch - Female Alpaca Details</title>
...
<%
  ...
  counter = 1
	...
	sql = "select * from FemaleWebView where Animals.ID=" & ID
	'response.write(sql)
	rs.Open sql, conn, 3, 3 
	if not rs.eof then 
 		photoID = rs("DetailPageImage")
 		If IsEmpty(photoID) or photoId = "0" then
            click = "<img   src=""uploads/DetailPage/NotAvailable.jpg""> "
        Else
				...
				<table border="0" cellspacing="0" width = "550" align = "center" valign = "top" >
				   <tr>
					...
					<% if counter < 1 then%>
							<%=click%>
						<% else %>
							<IMG alt="main image" border=0  name=but1 src="<%=buttonimages(1)%>" align = "center" >
						<% end if%>
					</td>
					<td  valign = "top" align = "center">
					<%
...
%>
			...
			<table border=0  valign=top cellspacing="3"  >
			  <tr>
				...
				<tr>
                    	...
                    	<td   class = "body">	<% if len(ARI) > 2 then%>
										<%=ARI%>
								<% end if %></td>
                </tr>	
				<tr>
				...
				<tr>
					<td width="60"  valign = "top"   class = "Details">
					<% if rs("Bred") = "True" then %>
						<br>Bred to:</td>
					<% end if%>	
	...
	<%
	  dim Hyperlinked
		Hyperlinked = 0
			if len(rs("ServiceSireID")) > 0 and not (rs("ServiceSireID")) = 0 then
												conns = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
			"Data Source=" & server.mappath("../db/AlpacaDB.mdb") & ";" & _
						"User Id=;Password=;" '& _ 
			sqls = "select * from WebView where Animals.ID =" & (rs("ServiceSireID"))
		...
		If len(ServiceSireImage) < 18 then 
				counter = counter +1
			     sireclick =  " <form action=""HerdsireDetails.asp"" method=""post"">" &_
                             "<input name=ID type=hidden value=" &  ServiceSireID & ">" & hiddenInput &_
                            "<input name=Detail type=image src=images/NotAvailable.jpg  border=0 width=""110"" > "
					Hyperlinked =1
							
			Else if len(ServiceSireImage) > 17 then
			counter = counter +1
   			     sireclick = "<form action=""HerdsireDetails.asp"" method=""post"">" &_
                             "<input name=ID type=hidden value=" &  ServiceSireID & ">" & hiddenInput &_
                             "<input name=Detail type=image src= "& ServiceSireImage & "  border=0 width=""100"" >"            
	...
	else if (rs("ExternalStudID")) > 0 then 
		'response.write(ExternalStudID)
			conn6 = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
			"Data Source=" & server.mappath("../db/AlpacaDB.mdb") & ";" & _
						"User Id=;Password=;" '& _ 
			sql6 = "select * from ExternalStud where ExternalStudID =" & (rs("ExternalStudID"))
			...
			if  len(CurrentXStudLink) > 1 then
					sireclick = "<a href = """& CurrentXStudLink &""" border = 0 target=""_blank""><img src ="""& CurrentXStudImage &""" height=""100""   border = 0 alt = Herdsire></a>"
					Hyperlinked = 1
			end if
			if  len(CurrentXStudLink) < 2 then
					sireclick = "<img src ="""& CurrentXStudImage &""" height=""100""   border = 0 alt = Herdsire>"
			end if
...
<table>
	<tr>
					<%if  Hyperlinked = 0 then%>
							<td valign = "top"><%= sireclick%></td>
							<td valign = "top">
