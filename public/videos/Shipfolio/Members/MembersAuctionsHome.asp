...
sql2 = "select Count(*) as count from AuctionDutch where PeopleId= '" & PeopleID & "' order by AuctionDutchID"
Set rs2 = Server.CreateObject("ADODB.Recordset")
rs2.Open sql2, conn, 3, 3 
if rs2.eof then 
numauctions = 0 
else
numauctions = cLng(rs2("count"))
end if

SubscriptionLevel = cLng(SubscriptionLevel)
...
<tr><td class = "body roundedBottom" align = "center" valign = "top">
...
$<input type=text onBlur="checkNumeric(this, -1,900000000,',','.','-');"  name='Animal1Ceiling(<%=rowcount%>)' size=6 maxsize=4 Value= "<%= Animal1Ceiling(rowcount)%>">
<% else %>
$<input type=text onBlur="checkNumeric(this, -1,900000000,',','.','-');"  name='Animal1Ceiling(<%=rowcount%>)' size=6 maxsize=4 Value= "">
<% end if %>
</td>
<td class = "body2" align = "center">
<% if len(Animal1Floor(rowcount)) > 0 then%>
$<input type=text onBlur="checkNumeric(this, -1,900000000,',','.','-');"  name='Animal1Floor(<%=rowcount%>)' size=6 maxlength=8 Value= "<%=Animal1floor(rowcount)%>">
<% else %>
$<input type=text onBlur="checkNumeric(this,-1,900000000,',','.','-');"  name='Animal1Floor(<%=rowcount%>)' size=6 maxlength=8 Value= "">
<% end if %>
