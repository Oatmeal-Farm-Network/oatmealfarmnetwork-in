...
rs.close
%>
<title><%=StateName %> Donkey Ranches | <%=StateName %> Donkey Farms | <%=StateName %> Donkey Breeders | Donkey Ranches in <%=StateName %> with Donkeys for sale.</title>
<meta name="Title" content="<%=StateName %> Donkey Ranches | <%=StateName %> Donkey Farms | <%=StateName %> Donkey Breeders | Donkey Ranches in <%=StateName %> with Donkey for sale."/>
<meta name="description" content="<%=StateName %> Donkey Ranches | Donkey Farms in <%=StateName %> with Donkey for sale." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> Donkey ranches,
<%=StateName %> Donkey Farms,
<%=StateName %> Donkey for sale,
<%=StateName %> Donkey breeders,
Donkey ranches in <%=StateName %>,
Donkey Farms in <%=StateName %>, 
Donkey for sale in <%=StateName %>,
Donkey breeders in <%=StateName %>" />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> Donkey farms" />
...
<H1><div align = "left"><%=StateName %> Donkey Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID = 7 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> donkey ranches listed with Livestock of America.  </b><br/></center>
...
rs.close
 ...
 Query =  "Select AdID, AdImage, AdLink from Ads  where AdType='Search' and AdMonth = " & month(now) & " and adYear=" & year(now)& " and speciesID = 7 ORDER BY rand() limit 12"
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Donkeys For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
