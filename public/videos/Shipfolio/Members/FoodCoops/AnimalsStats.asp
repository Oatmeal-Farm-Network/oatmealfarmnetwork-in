...
<%  sql = "select distinct AnimalID, AnimalName from Animalstats where PeopleID = " & Session("PeopleID") & " order by AnimalName"
response.write (sql)
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open sql, connStats, 3, 3  
...
sql = "SELECT animalID FROM Animalstats WHERE year(Statdate)=(year(DateAdd('y',1,now))-1) and AnimalID = " & ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
...
<% 
if showstats = true then
sql = "select animalID from StatsAnimalsTwoMonthsAgo where AnimalID = " &		ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
...
sql = "select animalID from StatsAnimalsLastMonth where AnimalID = " &		ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
		response.write(rs2.recordcount)
...
sql = "select animalID from StatsanimalsThisMonth where AnimalID = " &		ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
		response.write(rs2.recordcount)
...
sql = "select animalID from StatsanimalsLastWeek where AnimalID = " &		ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
		response.write(rs2.recordcount)
...
' if showstats = true then
	...
	sql = "SELECT animalID FROM Animalstats WHERE Statdate=DateAdd('d',-1,now) and AnimalID = " & ID(rowcount) 
	rs2.Open sql, connStats, 3, 3   	
	If Not rs2.eof then			
...
TotalCount=rowcount 
 ...
 %>
	...
	<tr bgcolor = "antiquewhite">
			...
			<td class = "body" align = "center" ><b>aaa<%=TotalHitsThisYear %></b></td>
