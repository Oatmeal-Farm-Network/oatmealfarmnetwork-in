...
<meta name="description" content="<%=StateName %> Alpaca Packages at <%= BusinessName %> in <%= AddressCity %>&nbsp; <%= AddressState %>. <%= BusinessName %> are alpaca breeders - Alpaca farmers. " />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="14 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %>Alpaca Packages, <%=StateName %> Alpacas for Sale " />
...
<br>
<% if TotalPrice > PackagePrice And TotalPrice > 22 Then %>
		<b>Full Price: <%= FormatCurrency(TotalPrice,0) %></b><br>
<% End If %>
<% if len(PackagePrice) > 1  then %>
<font color = "brown">Price: <b>
...
<% 
HideOBO = False
If PackageOBO = "True" And Sold = False and not HideOBO = True Then %><form  name=form method="post" action="PlacePackageOBObid.asp?PackageID=<%=PackageID%> ">
...
<% 	
sql = "SELECT distinct Animals.*, Pricing.*, Photos.*, Ancestors.*, Colors.*  FROM packageanimals, Animals, Pricing, Photos, Ancestors, Colors WHERE packageanimals.animalID = animals.ID and Animals.ID=Pricing.ID And Animals.ID=Colors.ID And Animals.ID=Photos.ID And Animals.ID=Ancestors.ID  And Animals.ID=Ancestors.ID and Packagetype = 'ForSale' and packageanimals.packageid = " & packageid & " and animals.PeopleID =" & CurrentPeopleID
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
If Not rs.eof then  %>
