...
rs.close
%>
<title><%=StateName %> Horse Ranches | <%=StateName %> Horse Farms | <%=StateName %> Horse Breeders | Horse Ranches in <%=StateName %> with Horse for sale.</title>
<meta name="Title" content="<%=StateName %> Horse Ranches | <%=StateName %> Horse Farms | <%=StateName %> Horse Breeders | Horse Ranches in <%=StateName %> with Horses for sale."/>
<meta name="description" content="<%=StateName %> Horse Ranches | Horse Farms in <%=StateName %> with Horse for sale." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> Horse ranches,
<%=StateName %> Horse Farms,
<%=StateName %> Horse for sale,
<%=StateName %> Horse breeders,
Horse ranches in <%=StateName %>,
Horse Farms in <%=StateName %>, 
Horse for sale in <%=StateName %>,
Horse breeders in <%=StateName %> />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> Horse farms" />
...
<H1><div align = "left"><%=StateName %> Horse Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID =5 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> Horse ranches listed with Livestock of America.  </b><br/></center>
...
Query =  "Select AdID, AdImage, AdLink from Ads  where AdType='Search' and AdMonth = " & month(now) & " and adYear=" & year(now)& " and speciesID = 5 ORDER BY rand() limit 12"
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Horses For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
