...
rs.close
%>
<title><%=StateName %> Pig Ranches | <%=StateName %> Pig Farms | <%=StateName %> Pig Breeders | Pig Ranches in <%=StateName %> with Pig for sale.</title>
<meta name="Title" content="<%=StateName %> Pig Ranches | <%=StateName %> Pig Farms | <%=StateName %> Pig Breeders | Pig Ranches in <%=StateName %> with Pig for sale."/>
<meta name="description" content="<%=StateName %> Pig Ranches | Pig Farms in <%=StateName %> with Pigs for sale." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> Pig ranches,
<%=StateName %> Pig Farms,
<%=StateName %> Pig for sale,
<%=StateName %> Pig breeders,
Pig ranches in <%=StateName %>,
Pig Farms in <%=StateName %>, 
Pig for sale in <%=StateName %>,
Pig breeders in <%=StateName %> />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> Pig farms" />
...
<H1><div align = "left"><%=StateName %> Pig Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID =12 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> Pig ranches registered with Livestock of America.  </b><br/></center>
...
End If  
%>
<a href = "ranchhome.asp?CurrentPeopleID=<%=PeopleID%>" class = "body"><img src = "<%=Logo %>" border = "0" width = "80" alt = "<%=AddressState%> Pigs for Sale at <%=BusinessName%> ranches."/></a>
<% End If %>
</td>
...
Query =  "Select AdID, AdImage, AdLink from Ads  where AdType='Search' and AdMonth = " & month(now) & " and adYear=" & year(now)& " and speciesID = 12 ORDER BY rand() limit 12"

rs.Open Query, conn, 3, 3
x = 0


while not rs.eof and x < 5
 x = x + 1
AdID = rs("AdID")
AdImage  = rs("AdImage") 
AdLink = rs("AdLink")
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Pig For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
