...
rs.close
%>
<title><%=StateName %> Sheep Ranches | <%=StateName %> Sheep Farms | <%=StateName %> Sheep Breeders | Sheep Ranches in <%=StateName %> with Sheep for sale.</title>
<meta name="Title" content="<%=StateName %> Sheep Ranches | <%=StateName %> Sheep Farms | <%=StateName %> Sheep Breeders | Sheep Ranches in <%=StateName %> with Sheep for sale."/>
<meta name="description" content="<%=StateName %> Sheep Ranches | Sheep Farms in <%=StateName %> with Sheep for sale." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> Sheep ranches,
<%=StateName %> Sheep Farms,
<%=StateName %> Sheep for sale,
<%=StateName %> Sheep breeders,
Sheep ranches in <%=StateName %>,
Sheep Farms in <%=StateName %>, 
Sheep for sale in <%=StateName %>,
Sheep breeders in <%=StateName %> />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> Sheep farms" />
...
<H1><div align = "left"><%=StateName %> Sheep Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID =10 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> Sheep ranches listed with Livestock of America.  </b><br/></center>
...
Query =  "Select AdID, AdImage, AdLink from Ads  where AdMonth = '" & month(now) & "' and adYear='" & year(now)& "' and  AdType='Search' and speciesID = 10 ORDER BY rand() limit 6"
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Sheep For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
