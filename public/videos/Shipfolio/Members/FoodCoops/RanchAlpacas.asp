...
CurrentBreedSingular = "Alpaca"
CurrentFile = "RanchAlpacas.asp"
...
if SortOrder = "DOB" or len(SortOrder) < 3 then Sortby = "DOBYear Desc, DOBMonth Desc"
...
<% 
sql = "select count(*) as TotalAnimalsForSale from animals where publishforsale = 1 and speciesID = " & SpeciesID & " and animals.PeopleID= " & CurrentPeopleID
rs.Open sql, conn, 3, 3
If not rs.eof then
    TotalAnimals = rs("TotalAnimalsForSale")
rs.close
End If 
 
sql = "select count(*) as TotalStuds from animals where PublishStud = 1 and speciesID = " & SpeciesID & " and animals.PeopleID= " & CurrentPeopleID
rs.Open sql, conn, 3, 3
If not rs.eof then
    TotalStuds = rs("TotalStuds")
rs.close
End If 
