...
%>
<title><%= BusinessName %> | <%=StateName %> Animals for Sale</title>
<meta name="Title" content="<%= BusinessName %> | <%=StateName %> Animals for Sale">
...
<% aboutuswidth = "700" %>
<table width = "720" border="0"  cellspacing="3" cellpadding="3" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" height = "200" align = "center">		<tr>
...
<table border = "0" cellspacing="0" cellpadding = "0" align = "center"  ><tr><td class = "roundedtop" align = "left" valign = "top">
...
<table border = "0" cellspacing="0" cellpadding = "0" align = "center" width = "<%=aboutuswidth +20%>" ><tr><td class = "roundedtop" align = "left">
...
<ul id="Ul1" class="LinkedList">
<%  If not rs.State = adStateClosed Then
rs.close
End If   
if CurrentPeopleID = 1016 then
 sql = "select distinct fullname, animals.ID, PriceComments, Price, Sold, salepending, studfee, discount, DOBDay, DOBMonth, DOBYear, color1, color2, color3, color4, color5,  Photo1, Breed, category, OBO, speciesID from Animals, Pricing, Colors, Photos where Animals.ID = Pricing.ID and Animals.ID = Photos.ID and Animals.ID = Colors.ID  and sold = false and  PublishForSale = true  and AGBrokered = True order by speciesID Desc, Price desc, FullName desc" 
else
  sql = "select distinct fullname, animals.ID, PriceComments, Price, Sold, salepending, studfee, discount, DOBDay, DOBMonth, DOBYear, color1, color2, color3, color4, color5,  Photo1, Breed, category, OBO, speciesID from Animals, Pricing, Colors, Photos where Animals.ID = Pricing.ID and Animals.ID = Photos.ID and Animals.ID = Colors.ID  and sold = false and  PublishForSale = true and PeopleID= " & CurrentPeopleID & " order by speciesID Desc, Price desc, FullName desc"
end if 
...
image = "http://www.AlpacaInfinity.com/Uploads/" & image
End If 
...
image = "http://www.AlpacaInfinity.com/" & image
End If 
else
image = "/images/logoTip.png"
end if
...
%>
<li><a class="tooltip" href="#"><b>&#8734;</b><span class="custom info"><% if len(image) > 20 then %><img src="<%=image %>" alt="<%=AnimalName %>" width="80"  /><% end if %>
...
Breed: <%=rs("Breed")%>&nbsp;<%=signularanimal %><br>
...
<a href = "Details.asp?ID=<%=rs("ID") %>&CurrentPeopleID=<%=CurrentpeopleID %>" class = "body"><%=rs("FullName") %></a>&nbsp;<%=rs("Breed")%> &nbsp;<%=signularanimal %>&nbsp; <%=rs("Color1") %>&nbsp;  <% if len(rs("Price")) > 2 then%>
	    <% if len(rs("Discount")) > 1   then %>
...
<table border = "0" cellspacing="0" cellpadding = "0" align = "center" >
<tr>
<td class = "roundedtopandbottom Ranchmenu" align = "left" width = "200">
...
Logo = "/uploads/" & Logo
End If %> 
<img src = "<%=Logo %>" width = "180" alt = "Alpacas for sale at <%=BusinessName %>" /><br />
<% else %>
<b><%=BusinessName %></b><br />
<% end if %>
<b><%=Owners %></b><br />
