...
rs.close
%>
<title><%=StateName %> Alpaca Ranches | <%=StateName %> Alpaca Farms | <%=StateName %> Alpaca Breeders | Alpaca Ranches in <%=StateName %> with alpacas for sale including huacaya and suri alpacas.</title>
<meta name="Title" content="<%=StateName %> Alpaca Ranches | <%=StateName %> Alpaca Farms | <%=StateName %> Alpaca Breeders | Alpaca Ranches in <%=StateName %> with alpacas for sale including huacaya and suri alpacas."/>
<meta name="description" content="<%=StateName %> Alpaca Ranches | Alpaca Farms in <%=StateName %> with alpacas for sale including huacaya and suri alpacas. White, beige, faun, brown, grey, and black alpacas." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> alpaca ranches,
<%=StateName %> alpaca Farms,
<%=StateName %> alpacas for sale,
<%=StateName %> alpaca breeders,
<%=StateName %> alpaka,
<%=StateName %> alpakas,
<%=StateName %> alpakas preise,
<%=StateName %> huacaya alpacas,
<%=StateName %> suri alpacas,
alpaca ranches in <%=StateName %>,
alpaca Farms in <%=StateName %>, 
alpacas for sale in <%=StateName %>,
alpaca breeders in <%=StateName %>,
alpakas in <%=StateName %>,
huacaya alpacas in <%=StateName %>,
suri alpacas in <%=StateName %>" />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> alpaca farms" />
...
<!--#Include virtual="/alpacas/AnimalTabsInclude.asp"-->
...
<H1><div align = "left"><%=StateName %> Alpaca Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID = 2 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> ranches listed with Livestock of America.  </b><br/></center>
...
rs.close
 ...
 Query =  "Select AdID, AdImage, AdLink from Ads  where AdType='Search' and AdMonth = " & month(now) & " and adYear=" & year(now)& " and speciesID = 2 ORDER BY rand() limit 12"
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Alpacas For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
