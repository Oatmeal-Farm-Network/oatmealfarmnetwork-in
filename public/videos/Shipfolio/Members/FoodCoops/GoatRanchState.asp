...
rs.close
%>
<title><%=StateName %> Goat Ranches | <%=StateName %> Goat Farms | <%=StateName %> Goat Breeders | Goat Ranches in <%=StateName %> with Goats for sale.</title>
<meta name="Title" content="<%=StateName %> Goat Ranches | <%=StateName %> Goat Farms | <%=StateName %> Goat Breeders | Goat Ranches in <%=StateName %> with Goat for sale."/>
<meta name="description" content="<%=StateName %> Goat Ranches | Goat Farms in <%=StateName %> with Goat for sale." />
<meta name="robots" content="index, follow"/>
<meta name="keywords" content="<%=StateName %> goat ranches,
<%=StateName %> Goat Farms,
<%=StateName %> Goat for sale,
<%=StateName %> Goat breeders,
goat ranches in <%=StateName %>,
goat Farms in <%=StateName %>, 
goat for sale in <%=StateName %>,
goat breeders in <%=StateName %> />
<meta name="robots" content="index, follow"/>
<meta name="rating" content="Safe for kids"/>
<meta name="revisit-after" content="7 days"/>
<meta name="Googlebot" content="index, follow"/>
<meta name="robots" content="All"/>
<meta name="subjects" content="<%=StateName %> Goat farms" />
...
<H1><div align = "left"><%=StateName %> Goat Ranches</div></H1>
...
<%
sql = "SELECT DISTINCT People.PeopleId, Logo, AddressCountry, PeopleFirstname, PeopleLastname, Business.BusinessName, AddressStreet, AddressApt, AddressCity, AddressState, AddressZip FROM People, Business, animals, address WHERE animals.PeopleID = people.peopleID and speciesID = 6 and People.AddressID = Address.AddressID  and People.BusinessID = Business.BusinessID and People.AIPublish = True and People.AccessLevel > 0 and addressstate = '" & state & "' ORDER BY Business.BusinessName;" 
'response.write (sql)
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3   
 
If rs.eof Then %><br/><br/>
	<center><b>Currently there are no <%=StateName %> Goat ranches listed with Livestock of America.  </b><br/></center>
...
rs.close
 ...
 Query =  "Select AdID, AdImage, AdLink from Ads  where AdType='Search' and AdMonth = " & month(now) & " and adYear=" & year(now)& " and speciesID = 6 ORDER BY rand() limit 12"
...
<% If Len(Link2) > 1  then%>
 ...
 <% end if %>
<img src = "<%= AdImage%>" width ="200" height = "200" border = "0" alt="Goats For Sale">
<% If Len(Link2) > 1  then%>
 <a>
 <% end if %>
<% End If %>
