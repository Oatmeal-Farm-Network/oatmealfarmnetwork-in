		...
		<table border = "0" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"  cellpadding=0 cellspacing=0>
			<tr>
				<td  valign = "middle">
					...
					<div align = "center">
					<Input type = text name='TotalCount' value = <%=TotalCount%> >
...
if len( SampleAge(rowcount)) < 1 and  len(Average(rowcount)) < 1  and len(StandardDev(rowcount)) < 1 and len(COV(rowcount)) < 1 and len(GreaterThan30(rowcount)) < 1 and len(CF(rowcount)) < 1 and  len(Curve(rowcount)) < 1 and len(ShearWeight(rowcount)) < 1 and len(BlanketWeight(rowcount)) < 1 and len(Length(rowcount)) < 1 and len(CrimpPerInch(rowcount)) < 1 then
blankcount = blankcount + 1
response.write("blankcount=" & blankcount )
end if
%>
