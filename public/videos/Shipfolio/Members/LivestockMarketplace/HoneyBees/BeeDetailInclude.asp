...
sql= sql & " FROM Animals, Pricing,  Photos, colors , address, People, SpeciesBreedLookupTable "
sql= sql & " WHERE animals.speciesID = " & speciesID & " and animals.PeopleID = People.PeopleID  and  animals.BreedID = SpeciesBreedLookupTable.BreedLookupID and People.AddressID = address.AddressID And pricing.id=Animals.id And Photos.id=Animals.id  and Colors.id=Animals.id and subscriptionlevel > 0 and PublishForSale = 1 and people.country_id = " & country_id & Categorysearch & Breedsearch & Colorsearch &  OBOSearch & currentmaxpriceSearch & currentminpriceSearch &  CategorySearch & Statesearch  
...
<% else %>
<div align = Right><%=count %> Results<br /></div>
<% end if %>
<% 
...
if speciesid = 25 or speciesid = 27 then
...
sql= sql & " FROM Animals, Pricing,  Photos, colors , address, People "
sql= sql & " WHERE animals.speciesID = " & speciesID & " and animals.PeopleID = People.PeopleID and People.AddressID = address.AddressID And pricing.id=Animals.id And Photos.id=Animals.id  and Colors.id=Animals.id and subscriptionlevel > 0 and PublishForSale = 1 and people.country_id = " & country_id & Categorysearch & Breedsearch & Colorsearch &  OBOSearch & currentmaxpriceSearch & currentminpriceSearch & CategorySearch & Statesearch  
sql= sql & " ) "
sql= sql & " SELECT * "
...
if trim(lcase(photoID))= "http://www.livestockofamerica.com/uploads/"  then
...
photoID = "http://www.livestockoftheworld.com/images/ImageNotAvailable.jpg"
end if 

if len(photoID) < 50 then
photoID = "http://www.livestockoftheworld.com/" & photoID
end if 
str1 = lcase(photoID)
...
<div class="container-fluid" align = "center" >
    ...
    <div class = "row nowrap" align = "center">
        <div class = "col-4 body box box2" style="min-width:250px; max-width: 250px; max-height: 220px">
       <a href = "/Animals/Details.asp?ID=<%=AnimalID%>&DetailType=<%=DetailType%>&CurrentPeopleID=<%=CurrentpeopleID %>" >
           <img src = "<%= PhotoID %>" alt = "Image" style="border:1px solid black;" width="200px" align="center" ></a>
        </div>
