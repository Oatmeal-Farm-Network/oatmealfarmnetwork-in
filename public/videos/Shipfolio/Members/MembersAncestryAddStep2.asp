...
SubmitType = request.Form("bsubmit")
response.write("SubmitType = " & SubmitType)
...
'response.write("AnimalID = " & AnimalID & "<br>")
Name =Request.Form("Name") 

AncestryDescription = Request.Form("AncestryDescription")
...
%>
<!--#Include File="MembersCheckAncestorDataInclude.asp"-->
<%
Dam   = Ancestor
Dam2   = Ancestor2
...
DamSireSireARI  = AncestorARI
 ...
 Ancestor = Sire 
 'response.write("Ancestor=" & Ancestor)
 Ancestor2 = Sire2  
...
sql = "select * from animals, Colors, AnimalRegistration where animals.AnimalID = Colors.AnimalID and animals.AnimalRegistrationID = AnimalRegistration.AnimalRegistrationID and FullName =  '" & Dam2 & "'"
 rs.Open sql, conn, 3, 3 
 if not rs.eof then
 DamColor = rs("Color1") 
 DamARI = rs("RegNumber")
 DamCLAA = rs("CLAA")
 damLink = rs("ExternalLink")
 if len(rs("Color2")) > 1 then
...
rs.close
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & Dam2 & "'"
...
End If
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & DamDam2 & "'"
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
 response.write(sql)
    DamDam = DamDam2  
...
End If
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & DamSire2 & "'"
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
...
sql = "select * from animals, Colors, AnimalRegistration where animals.AnimalID = Colors.AnimalID and animals.AnimalRegistrationID = AnimalRegistration.AnimalRegistrationID and FullName =  '" & Sire2 & "'"
 rs.Open sql, conn, 3, 3 
 if not rs.eof then
 SireColor = rs("Color1") 
 SireARI = rs("RegNumber")
 SireCLAA = rs("CLAA")
 SireLink = rs("ExternalLink")
 if len(rs("Color2")) > 1 then
...
rs.close
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & Sire2 & "'"
 response.Write("sql=" & sql)
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
...
End If
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & SireDam2 & "'"
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
...
End If
 ...
 sql = "select animals.AnimalID, ancestors.* from animals, Ancestors where animals.AnimalID = Ancestors.AnimalID and FullName =  '" & SireSire2 & "'"
 Set rs = Server.CreateObject("ADODB.Recordset")
 rs.Open sql, conn, 3, 3 
...
Query =  " UPDATE Animals Set AncestryDescription = '" &  AncestryDescription & "' " 
Query =  Query & " where AnimalID = " & AnimalID & ";" 
...
response.write("BusinessID=" & BusinessID & "<br><br>")

if SubmitType = "Next" or SpeciesID = 18 or SpeciesID = 13 or SpeciesID = 21 or SpeciesID = 30 or speciesID = 19 or SpeciesID = 23 or SpeciesID = 26 or SpeciesID = 28 or SpeciesID = 33 or NumberofAnimals > 1 then
submitstring="MembersAnimalAdd3.asp?BusinessID=" & BusinessID & "&Animalid=" & AnimalID & "&Name=" & Name  & "&SpeciesID=" & SpeciesID & "&NumberofAnimals=" & NumberofAnimals
response.write("submitstring=" & submitstring )

 response.redirect(submitstring )
  else
'response.redirect("MembersAnimalAdd2.asp?BusinessID=" & BusinessID & "AnimalID=" & AnimalID & "&Name=" & Name  & "SpeciesID=" & SpeciesID)
end if
%>
 </Body>
</HTML>
