...
if len(prodCategory2ID) > 0 then
    ...
    sql = "select CatName from SFCategories where CatID= " & prodCategory2ID
	Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open sql, conn, 3, 3 
        if not rs.eof then
...
if len(prodCategory3ID) > 0 then
    ...
    sql = "select CatName from SFCategories where CatID= " & prodCategory3ID
	Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open sql, conn, 3, 3 
        if not rs.eof then
...
<% end if %>
        ...
        Price <font color="#abacab">(Optional)</font><br>
             $<input name="ProdPrice" type="number"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" value="<%=ProdPrice%>" class="currency formbox" id="c1" style="width: 140px; max-height:28px; text-align: left"/><br>
            <i><font color = "#404040"><small>Must be a number, and greater than $0.</small></font></i>
        </div>
        <div class="col body" style="height: 100px">
        Sale Price <font color="#abacab">(Optional)</font><br>
           $<input name="SalePrice" type="number"  min="0" step="0.01" data-number-to-fixed="2" data-number-stepfactor="100" value="<%=SalePrice%>" class="currency formbox" id="c1" style="width: 140px; max-height:28px; text-align: left" id="c1" /><br>
            <i><font color = "#404040"><small>Must be a number, and greater than $0.</small></font></i>
        </div>
        <div class="col body" style="height: 100px">
            Custom Order? <font color="#abacab">(Optional)</font></font><br />
        ...
        <% if len(ProdQuantityAvailable) < 1 then
         ...
         <input name="ProdQuantityAvailable" type = number value="<%=ProdQuantityAvailable%>" size = "20" class = "formbox" id="c1" style="width: 140px; text-align: left"> 
        </div>
        <div class="col body" style="height: 70px">
...
<div align = "center"><input type=submit value = "Submit" class = "submitbutton"  <%=Disablebutton %> ></div>
</form>
<br><br><br>
</div>
</div>
