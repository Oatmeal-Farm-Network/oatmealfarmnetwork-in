...
Query =  Query & " Values (" &  PageLayoutID & ", '" & ProdServiceID & "' , 'Service', " & textblock & ")"
...
<form method="POST" action="AdminPageEditServicesInclude.asp?PageLayoutID=<%= PageLayoutID %>&textblock=<%= textblock %>&Add=True" >
<b>Add Service:</b>  
<select name="ProdServiceID" onchange="submit();">
<option value="">Select to Include a Service</option>
<% sql = "select * from Services " 
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3 
while Not rs.eof %>
<option value="<%=rs("ServicesID") %>"><%=rs("ServiceTitle") %></option>
<% rs.movenext
wend 
rs.close
%>
...
<% sql = "select * from ProdServiceReferenceTable, services  where services.PageLayoutID = " & PageLayoutID  & " and ProdServiceIDType = 'Service' and ProdServiceReferenceTable.ProdServiceID = Services.servicesID and textblock = " & textblock & " order by ProdServiceReferenceID DESC " 
...
<Tr><td class = "body"><center><b>Service</b></center></td>
<td class = "body"><center><b>Price</b></center></td>
<td></td>
</Tr>
<% while not rs.eof %>
 <tr>
...
<%=rs("ServiceTitle") %>
    ...
    <td class = "body" align = 'center'><center><% ServicePrice = rs("ServicePrice") %>
 <%= rs("ServicePrice") %>
     </td>
<td align = 'center'> 

<form method="POST" action="AdminPageEditServicesInclude.asp?PageLayoutID=<%= PageLayoutID %>&textblock=<%= textBlock %>&Delete=True" > <input type=submit value = "X" class = "regsubmit2"  <%=Disablebutton %> ><input name="ProdServiceReferenceID" value="<%=rs("ProdServiceReferenceID") %>" type = "hidden"></td>
  </tr></form>
<% rs.movenext
wend  
end if
