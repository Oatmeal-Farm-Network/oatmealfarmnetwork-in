...
<tr><td bgcolor = "#F2F2F2" height = 40 colspan = 2><div align = center><form action = '/Ranches/RanchContactUs.asp?CurrentPeopleID=<%=CurrentPeopleID %>&screenwidth=<%=screenwidth %>' method = "post" >
...
<% If not (SpeciesSalesType = "Fowl" and numberofanimals > 1) then %>
...
<% If Len(price) > 2 and not (dutchauction = True) Then %>
<tr><td bgcolor = "#F2F2F2" height = 1 colspan = 2></td></tr>
...
<% if len(trim(Vaccinations)) > 0 then %>
 <tr><td bgcolor = "#F2F2F2" height = 1 colspan = 2></td></tr>
<tr><td class = "body" height = 30>Vaccinations</td>
<td class = "body"><%=Vaccinations%> </td>
</tr>
<% end if %>
...
if noqty="True" then %>
<tr><td bgcolor = "#F2F2F2" height = 1 colspan = 2>
<b><font color = maroon>Please select a quantity.</font></b><br />
</td></tr>
<% end if %>
...
<tr>
<td class = "body" valign = "top">Order</td>
<td class = "body">


<form name="form" action="FowlPaymentForwardtopaypal.asp?ID=<%=ID %>" method="post">


<table width = 320 cellpadding = 0 cellspacing = 0>

<% if Price1 > 0  and not (MinOrder1 > (MaxOrder1 - 1))then 
...
%>
<tr><td colspan = 3 bgcolor = #e6e6e6 height = 1></td></tr>
<tr>
<td class = "body">
<b>Straight Run <%=pluralanimal %></b>
</td>
<td class = "body">
<%=formatcurrency(Price1, 2) %>
</td>
<td class= "body">
<input type="hidden" name="item_name_<%=itemcounter %>"  value="Straight Run <%=pluralanimal %>">
<input type="hidden" name="amount_<%=itemcounter %>" value="<%=Price1 %>">

<select size="1" name="quantity_<%=itemcounter %>" width="80" style="width: 80px" class = "formbox">
<option value="" selected><font face = "verdana" size = "1"> - </font>
<% count = MinOrder1
while count < MaxOrder1
count = count + 1 %>
...
<% if Price2 > 0 and not (MinOrder2 > (MaxOrder2 - 1)) then 
...
<tr>
<td class = "body">
<b>Female <%=pluralanimal %></b>
</td>
<td class = "body">
<%=formatcurrency(Price2, 2) %>
</td>
<td class= "body">
<input type="hidden" name="item_name_<%=itemcounter %>" value="Female <%=pluralanimal %>">

<input type="hidden" name="amount_<%=itemcounter %>" value="<%=Price2 %>">

<select size="1" name="quantity_<%=itemcounter %>" width="80" style="width: 80px"  class = "formbox">
<option value="" selected><font face = "verdana" size = "1"> - </font>
<% count = MinOrder2
while count < MaxOrder2
count = count + 1 %>
<option value="<%=count %>"><font face = "verdana" size = "1"><%=count %></font>
</option>
<% 	
...
<% if Price3 > 0 and not (MinOrder3 > (MaxOrder3 - 1)) then 
...
<input type="hidden" name="amount_<%=itemcounter %>" value="<%=Price3 %>">

<select size="1" name="quantity_<%=itemcounter %>" width="80" style="width: 80px" class = "formbox">
<option value="" selected><font face = "verdana" size = "1"> - </font>
<% count = MinOrder3
while count < MaxOrder3
count = count + 1 %>
<option value="<%=count %>"><font face = "verdana" size = "1"><%=count %></font>
</option>
<% 	
...
<% if Price4 > 0 and not (MinOrder4 > (MaxOrder4 - 1)) then 
...
<td class= "body">
<input type="hidden" name="item_name_<%=itemcounter %>" value="Eggs">
<input type="hidden" name="amount_<%=itemcounter %>" value="<%=Price4 %>">

<select size="1" name="quantity_<%=itemcounter %>" width="80" style="width: 80px" class = "formbox">
<option value="" selected><font face = "verdana" size = "1"> - </font>
<% count = MinOrder4 
while count < MaxOrder4
count = count + 1 %>
<option value="<%=count %>"><font face = "verdana" size = "1"><%=count %></font>
</option>
<% 	
...
<center><input type=submit value = "Buy Now"  class = "regsubmit2" ></center>
</form>
<br />
...
<tr><td colspan = 2 bgcolor = #e6e6e6 height = 1></td></tr>
<tr><td>
