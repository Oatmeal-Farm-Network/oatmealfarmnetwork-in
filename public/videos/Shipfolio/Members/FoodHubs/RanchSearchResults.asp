...
Function IsValidUrl(ByVal strUrl)
    Dim regEx, match
    ' Regular expression for URL validation
    Set regEx = New RegExp
    regEx.IgnoreCase = True
    regEx.Pattern = "^(https?://)?(www\.)?([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(/.*)?$"
    Set match = regEx.Execute(strUrl)
...
Function ConvertToValidUrl(ByVal strUrl)
    ' Check if the URL starts with http:// or https://, if not, add http://
    If Left(LCase(strUrl), 7) <> "http://" And Left(LCase(strUrl), 8) <> "https://" Then
        ConvertToValidUrl = "http://" & strUrl
    Else
        ConvertToValidUrl = strUrl
    End If
End Function

websiteAddress = Request.Form("websiteAddress")

' Check if the websiteAddress is valid
If IsValidUrl(websiteAddress) Then
    ' Convert the websiteAddress to a valid URL if necessary
    websiteAddress = ConvertToValidUrl(websiteAddress)
    
    ' Process the valid URL (store in DB, display, etc.)
    'Response.Write "Valid URL: " & websiteAddress
Else
...
sql = "SELECT DISTINCT Business.*, Address.*, People.custLastAccess, Websites.* " &_
"FROM Business " &_
"JOIN Address ON Business.addressID = Address.AddressID " &_
"JOIN People ON People.BusinessID = Business.BusinessID " &_
"JOIN Websites ON Business.WebsitesID = Websites.WebsitesID " &_
"WHERE Business.BusinessTypeID = " & BusinessTypeID & " AND Address.country_id = '" & Currentcountry_id & "' " & statesort & citysort & namesort & ownersort & " ORDER BY People.custLastAccess DESC, Business.BusinessName, Business.Logo ASC"
...
BusinessFacebookValid=IsValidUrl(BusinessFacebook )

If IsValidUrl(BusinessFacebook) = True then
    str1 = BusinessFacebook
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
            BusinessFacebook = "https://" & str1
        End If
end if

If IsValidUrl(BusinessX) = True then
    str1 = BusinessX
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
         BusinessX = "https://" & str1
        End If
end if


If IsValidUrl(BusinessInstagram) = True then
    str1 = BusinessInstagram
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessInstagram = "https://" & str1
        End If
end if



If IsValidUrl(BusinessPinterest) = True then
    str1 = BusinessPinterest
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessPinterest = "https://" & str1
        End If
end if
...
If IsValidUrl(BusinessTruthSocial) = True then
    str1 = BusinessTruthSocial
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessTruthSocial = "https://" & str1
        End If
end if


If IsValidUrl(BusinessBlog) = True then
    str1 = BusinessBlog
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessBlog = "https://" & str1
        End If
end if

If IsValidUrl(BusinessYouTube) = True then
    str1 = BusinessYouTube
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessYouTube = "https://" & str1
        End If
end if

If IsValidUrl(BusinessOtherSocial1) = True then
    str1 = BusinessOtherSocial1
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessOtherSocial1 = "https://" & str1
        End If
end if


If IsValidUrl(BusinessOtherSocial2) = True then
    str1 = BusinessOtherSocial2
        If Left(LCase(str1), 7) <> "http://" AND Left(LCase(str1), 8) <> "https://" Then
        BusinessOtherSocial2 = "https://" & str1
        End If
end if
    ...
    <div class="row">
        ...
        <div class="col body" style="vertical-align: top;">
            ...
            <% if len(PeopleWebsite) > 4 then %>
                <a href="<%=PeopleWebsite%>" class="body" target="_blank"><%=PeopleWebsite  %></a><br />
            <% end if %>
            ...
            <br />
              ...
              <% end if %>
                    ...
                    <%
                    If BusinessFacebookValid=True then %>
