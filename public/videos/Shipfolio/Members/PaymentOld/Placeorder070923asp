...
Function AddCustomer(name, email, phone, description)
	 ...
	 if name <>"" and email <>"" and stripeToken<>""  Then
		Dim customerDetails, requestBody,responseText, AttachPaymentMethodStatus
		...
		AttachPaymentMethodStatus=	AttachPaymentMethod (StripeCustomerID ,StripePaymentMethodID)
		
		session("ErrorMessage" ) = "AttachPaymentMethodStatus=" & AttachPaymentMethodStatus

		If InStr(AttachPaymentMethodStatus, "Error") > 0 Then
			Response.Write AttachPaymentMethodStatus
		else
			...
			if(StripeSubscriptionID <> "Incomplete") then
				Response.Write "success|"& StripeCustomerID &"|"& StripeSubscriptionID
			else
				Response.Write "Subscription Failed, Enter Correct CC Details"
			end if
...
Function AttachPaymentMethod(StripeCustomerID,StripePaymentMethodID)
	Dim payment_methodsAttachURL, PaymentMethodAttachedID
	...
	AttachPaymentMethod=GetIdFromJson(responseText) 
End Function
...
Function AddSubscription(StripeCustomerID,StripePriceID)
	dim subscriptionURL, CustomerIDSub, finalStatus
	...
	finalStatus=GetStatusFromJson(responseText)	
	if(finalStatus="active") then
		AddSubscription=GetIdFromJson(responseText)
	else
		AddSubscription="Incomplete"
	end if
...
Function GetIdFromJson(strJson)	
	...
	dim recordID
	If InStr(strJson, "doc_url") > 0 Then
		recordID="Error " & oJSON.data("error").item("message")



	else
		recordID= oJSON.data("id")
	end if
	GetIdFromJson=recordID

		

End Function
Function GetStatusFromJson(strJson)
	
	Set oJSON = New aspJSON
	oJSON.loadJSON(strJson)
	GetStatusFromJson= oJSON.data("status") 
