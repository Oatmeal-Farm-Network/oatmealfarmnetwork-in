...
<!--#Include file="MembersMobileWidthInclude.asp"-->
...
sqla = "select count(*) as numPublishedproducts from sfproducts where ProdForSale = True and PeopleID = " & tempPeopleID & " Order by ProdPrice desc"
rsa.Open sqla, conn, 3, 3
If not rsa.eof then
numPublishedproducts = cint(rsa("numPublishedproducts"))   
if numPublishedproducts > 5 and Subscriptionlevel < 4 then
...
rsa.close
sqla = "select count(*) as numPublishedAnimals from animals, Pricing where animals.id = Pricing.id and PublishForSale = True and PeopleID = " & tempPeopleID & " Order by Price desc"
rsa.Open sqla, conn, 3, 3
If not rsa.eof then
numPublishedAnimals = cint(rsa("numPublishedAnimals"))   
if numPublishedAnimals > 5 and Subscriptionlevel < 4 then
