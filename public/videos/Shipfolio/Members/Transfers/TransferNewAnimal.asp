...
rs.Open sql, DestinationConn, 3, 3   
If Not rs.eof Then
...
Query = Query & " ExternalLink, Description1, StudDescription1, Owner )" 

Query =  Query & " Values (" &  SourcePeopleID & " ,"
...
DestinationConn.Execute(Query) 
'
'set DestinationConn = Nothing %>
 
<% Found = False
sql = "select ID from Animals where  peopleID= " & SourcePeopleID & " order by ID Desc;" 
rs.Open sql, DestinationConn, 3, 3  
AnimalID = rs("ID") 
If rs.eof Then
else
...
End If
Query =  "INSERT INTO Awards(ID, Show, "
...
For rowcount = 1 To 21
				Query =  "INSERT INTO Fiber(ID, SampleAge, Average, StandardDev, COV, GreaterThan30, CF,  Curve, Shearweight,  BlanketWeight, Length, CrimpPerInch, LargeHistogram, SmallHistogram )" 
				Query =  Query & " Values (" &  AnimalID & ", "
