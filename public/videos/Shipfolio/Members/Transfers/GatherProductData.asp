<% Administration= True
Transfer = False %><!--#Include virtual="/Conn.asp"--> 
<% 
...
dim ShippingToCountryArray(1000) 
dim ShippingCost1Array(1000) 
dim ShippingCost2Array(1000) 
'response.write(ID)]
SourceProdID  = ProdID 
x = 0
...
while not rs.eof
ShippingToCountryArray(x) = rs("ShippingToCountry")
ShippingCost1Array(x) = rs("ShippingCost1")
ShippingCost2Array(x) = rs("ShippingCost2")
x = x + 1
rs.movenext
wend
rs.close
...
if not rs.eof then
	SourceCategory1ID = rs("prodcategoryId")
...
if not rs.eof then
SourceCategory2ID =  rs("prodcategoryId")
...
if not rs.eof then
SourceCategory3ID =  rs("prodcategoryId")
