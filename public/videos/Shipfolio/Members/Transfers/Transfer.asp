...
sql = "select AnimalRegistrationID from Animalregistration where  RegNumber= '" &  SourceARI & "' Order by AnimalRegistrationID DESC  ;" 
'response.write("SQL = " & sql & "<br><br>")
Set rs = Server.CreateObject("ADODB.Recordset")
rs.Open sql, conn, 3, 3 
If Not rs.eof Then
    AnimalRegistrationID = rs("AnimalRegistrationID")
    'response.Write("AnimalRegistrationID = " & AnimalRegistrationID & "<br><br>")
End if
 rs.close
...
'response.write("XXX Query = " & Query & "<br>")	
