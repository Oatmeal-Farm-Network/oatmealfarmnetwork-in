...
<% 
     sql = "select animals.*, Pricing.*, colors.*, photos.*, ancestors.* from Animals, Pricing, Colors, photos, ancestors  where Animals.ID = Pricing.ID and animals.id = ancestors.id and animals.id = photos.id and animals.id = colors.id and PeopleID = " & Session("PeopleID") & " and animals.id=" & ID 
	'response.write (sql)
