...
sql = "select SpeciesID, NumberOfAnimals from Animals where ID=" & ID
...
<a name="Fiber"></a>
 <table border = "0" cellspacing="0" cellpadding = "0" align = "center"   ><tr><td class = "roundedtop" align = "left">
        ...
        <tr><td class = "roundedBottom" align = "center" valign = "top" width = "90%">
<%
...
Conn.Execute(Query) 
	...
	sql = "select Animals.FullName, Fiber.* from Animals, Fiber where Animals.ID = Fiber.ID and animals.ID = " & ID & " Order by SampleDateYear desc,  SampleDateMonth desc, Average"
...
<input type = "hidden" name="IDArray(<%=rowcount%>)"  value="<%=ID%>" ><input type = "hidden" name="FiberID(<%=rowcount%>)" value="<%=FiberID(rowcount)%>" ><input type = "hidden" name="FullName(<%=rowcount%>)"  ><tr >
		<td width = "160" class = "body" align= "left">Sample Date:<br />
			...
			<select size="1" name="SampleDateMonth(<%=rowcount%>)" CLASS = "formbox">
		<%  if len(SampleDateMonth(rowcount)) > 0 then %>
			<%  if SampleDateMonth(rowcount) > 0 then %>
				...
				</select>/<select size="1" name="SampleDateDay(<%=rowcount%>)" CLASS = "formbox">
				...
				</select>/<select size="1" name="SampleDateYear(<%=rowcount%>)" CLASS = "formbox">
...
<td class = "body" align = "left" >
		   <input name="StandardDev(<%=rowcount%>)"  size = "5" value="<%=StandardDev(rowcount)%>" CLASS = "formbox"><br />
		  <input name="CrimpPerInch(<%=rowcount%>)" value="<%= CrimpPerInch(rowcount)%>" size = "5" CLASS = "formbox">
		</td>
		...
		<td class = "body" align = "left" >
			<input name="COV(<%=rowcount%>)" value="<%= COV( rowcount)%>"  size = "5" CLASS = "formbox"><br>
			<input name="Length(<%=rowcount%>)" value="<%= Length(rowcount)%>"  size = "5" CLASS = "formbox">
		</td>
		<td class = "body2" align = "right" >
			% > 30:<br />
			Shear Wt:</td>
		<td class = "body"  align = "right">
		 <input name="GreaterThan30(<%=rowcount%>)" value="<%= GreaterThan30(rowcount)%>"  size = "5" CLASS = "formbox"><br>
		 <input name="ShearWeight(<%=rowcount%>)" value="<%= ShearWeight(rowcount)%>"   size = "5" CLASS = "formbox">
		</td>
		<td class = "body2"  align = "right">
		 Curve:<br>
		 Blanket Wt:</td>
		<td class = "body"  align = "right">
		<input name="Curve(<%=rowcount%>)" value="<%= Curve(rowcount)%>"  size = "5" CLASS = "formbox"><br>
		<input name="BlanketWeight(<%=rowcount%>)" value="<%= BlanketWeight(rowcount)%>" size = "5" CLASS = "formbox">
		</td>
...
'response.write(TotalCount)
	rs.close
%>
<table width = "100%" border = "0"  leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"  cellpadding=0><tr><td align = "Right">
