...
'response.write("<br>ExistingAccount=" & ExistingAccount)



if ExistingAccount = False then
	Query =  "INSERT INTO People (PeopleFirstName, PeopleLastName, Peopleemail)" 
	Query =  Query & " Values ('" & PeopleFirstName & "', '" & PeopleLastName & "' , '" & Peopleemail & "' )" 

	Conn.Execute(Query) 
	
	sql = "select PeopleID from People where PeopleFirstName = '" & PeopleFirstName & "' order by PeopleID Desc"
...
includesub=false
if len(PeopleID)> 0 and ExistingAccount = False then 
...
<div class="form-group"><br />
	...
	<input type="text" id="numericInput" oninput="validateInput(event)" title="PeoplePhone" name="PeoplePhone" class="form-control" required>
