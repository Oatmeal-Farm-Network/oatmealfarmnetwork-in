...
<div>
	<div style=" min-height:60px">
        <br /><b>&nbsp;&nbsp;&nbsp;Your Changes Have Been Made.</b><br>
	</div>
</div>
<% end if %>
...
<div class ="row" style="background-color:white; border:0; padding:0; margin:0">
    <div class ="col body">
        <form name="myform" method="post" action= 'MembersServicesUpdate.asp?ServicesPagelayoutID=<%=ServicesPagelayoutID %>' >
        <input name="ServicesID" value="<%=ServicesID %>" type = "hidden">
        <input type = "hidden" name="ServiceCategoryID" value="<%=ServiceCategoryID %>">

        Service Title<br />
        <input type = "text" name="ServiceTitle" value="<%=ServiceTitle %>" class = "formbox" style="width:350px; min-height:40px; text-align:left" required>
    </div>
</div>
<%=HSpacer %>
<div class ="row">
...
<textarea name="ServicesDescription" ID="ServicesDescription" rows="6" class="formbox"  ><%=ServicesDescription%></textarea>
<br>
	<div align="right"><input type=submit value = "Update" class = "regsubmit2"  <%=Disablebutton %> ></div><br /><br />
    </form>
