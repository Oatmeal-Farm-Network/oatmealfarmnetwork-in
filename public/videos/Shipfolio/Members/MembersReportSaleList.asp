...
<%=AddressCity%>, &nbsp;<%=AddressState %>&nbsp; <%=Zip%><br>
<% end if %></font>
...
<% 	' Get marketing text for the top of the page:
      sql = "select animals.*, Pricing.*, colors.*, photos.*, ancestors.* from Animals, Pricing, Colors, photos, ancestors  where Animals.ID = Pricing.ID and animals.id = ancestors.id and animals.id = photos.id and animals.id = colors.id and PeopleID = " & Session("PeopleID") & " and (Category = 'Experienced Female' or Category = 'Inexperienced Female'  ) and forsale = true order by FullName"
...
<% 	     
     sql = "select animals.*, Pricing.*, colors.*, photos.*, ancestors.* from Animals, Pricing, Colors, photos, ancestors  where Animals.ID = Pricing.ID and animals.id = ancestors.id and animals.id = photos.id and animals.id = colors.id and PeopleID = " & Session("PeopleID") & " and (Category = 'Experienced Male' or Category = 'Inexperienced Male'  ) and forsale = true order by FullName"
...
<% 	     
  sql = "select animals.*, Pricing.*, colors.*, photos.*, ancestors.* from Animals, Pricing, Colors, photos, ancestors  where Animals.ID = Pricing.ID and animals.id = ancestors.id and animals.id = photos.id and animals.id = colors.id and PeopleID = " & Session("PeopleID") & " and ( Category = 'non-breeder'  ) and forsale = true order by FullName"
