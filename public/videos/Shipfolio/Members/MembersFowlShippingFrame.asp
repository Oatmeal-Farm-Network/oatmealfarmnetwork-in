...
<link rel="stylesheet" type="text/css" href="membersFramestyle.css">
...
%>
<% if len(ProdId)> 0 then %>
<form method="POST" action="membersShippingFrame.asp?prodid=<%= Prodid %>&Update=True" > 
<% else %>
<form method="POST" action="membersShippingFrame.asp?Servicesid=<%= Servicesid %>&Update=True" > 
<% end if %>
...
<% if len(ProdId)> 0 then %>
 ...
 <% else %>
  <form method="POST" action="membersShippingFrame.asp?Servicesid=<%= Servicesid %>&Delete=True" >
 <% end if %>
...
<blockquote><img src="images/Important_Triangle.png" height = 20 /> <b>If you do not set any shipping and handling costs then buyers will only be able to order your product, in your home country, and at the standard base price and attributes will not be displayed.</b><br /><br />
