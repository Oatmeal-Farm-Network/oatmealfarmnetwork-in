...
<%
title= "Cattle Breeds - W,X,Y,Z" 
image = "/Livestockdb/Cattle/Cattleheader.jpg"  %>
<Title><%=Title %></Title>
...
sql2 = "select * from SpeciesBreedLookupTable where breedavailable = 1  and SpeciesID=8 and ( left((trim(LOWER(breed))), 1) = 'w' or left((trim(LOWER(breed))), 1) = 'x' or left((trim(LOWER(breed))), 1) = 'y' or left((trim(LOWER(breed))), 1) = 'z' ) Order by trim(Breed)"
