...
<% 
Description = "Rabbits are small cute furry critters found in several parts of the world. There are eight different groups within the rabbit family, including the European rabbit, cottontail rabbits, and the Amami rabbit (an endangered species in Japan).The only rabbit to be widely domesticated is the European rabbit, which has been extensively bred for food and as pets. Rabbits were first widely kept in ancient Rome and they were refined into a wide variety of breeds during and since the middle Ages.<br /><br />Domesticated rabbits have mostly been bred to be much larger than wild rabbits, although selective breeding has produced rabbits in a wide range of sizes from dwarf to giant."
Title = "Rabbit Breeds"
image = "RabbitHeader.webp"
AboutLink = "AboutRabbits.asp"
letter = "x, y, and z"
 %>
...
<a name="Breeds"></a>
<h1><img src="<%=BreedIcon %>" alt="About <%= trim(Breed) %> <%=SpeciesNamePlural %>" height="40" />&nbsp;Breeds of <%=SpeciesNamePlural %> - <%=ucase(letter) %></h1>
...
<br />
  ...
  <%
  Set rs2 = Server.CreateObject("ADODB.Recordset")
  sql2 = "select Breed, BreedLookupID, BreedDescription, BreedImage, BreedVideo, BreedImageOrientation, BreedImageCaption from SpeciesBreedLookupTable where (left((trim(LOWER(breed))), 1) = 'x' or left((trim(LOWER(breed))), 1) = 'y' or left((trim(LOWER(breed))), 1) = 'z' ) and SpeciesID=" & speciesID & " Order by trim(Breed)"
  rs2.Open sql2, conn, 3, 3
