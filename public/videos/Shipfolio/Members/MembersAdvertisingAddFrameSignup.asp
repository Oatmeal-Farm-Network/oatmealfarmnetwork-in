...
TotalMegaAds=0
PurpleDutchAuctions = 0
BlueDutchAuctions = 0
RedDutchAuctions = 0
YellowDutchAuctions = 0
...
Query =  "delete * From Ads where PeopleID = " &  PeopleID & " and adWebsite = 'Alpaca Infinity' and AdPaidFor=False "
		Conn.Execute(Query) 
Conn.close
set Conn = Nothing %>	
...
<%
if discount > 0 then 
    TotalOrder = formatcurrency(85 - 85*Discount/100,0) 
 else
    TotalOrder = formatcurrency(85,0) 
 end if 

TotalmembershipOrder = TotalOrder
 Membershipcount =1
 if discount > 0 then 
...
TotalEmailOrder = Emailquantity1 * EmailPrice
 ...
 if discount > 0 then 
    EmailPrice2 = formatcurrency(150 - 150*Discount/100,0) 
 else
  EmailPrice2 = formatcurrency(150,0) 
 end if 
...
TotalAccountSetup2Order = AccountSetup2 * AccountSetup2Price
 ...
 if discount > 0 then 
    WebringPrice = formatcurrency(62 - 62*Discount/100,0) 
 else
  WebringPrice = formatcurrency(62,0) 
 end if 
WebRingcount = Request.Form("WebRingCheck")
if len(WebRingcount) > 1 then
    TotalWebRingOrder = WebringPrice
    TotalOrder = TotalOrder + WebringPrice
end if


 if discount > 0 then 
    AccountSetupPrice = formatcurrency(150 - 150*Discount/100,0) 
    else
      AccountSetupPrice = formatcurrency(150,0) 
 end if 
AccountSetupcount = Request.Form("AccountSetupCheck")
if len(AccountSetupcount) > 1 then
    TotalAccountSetup = AccountSetupPrice
    TotalOrder = TotalOrder + AccountSetupPrice
end if

if discount > 0 then 
    PurpleAuctionPrice = formatcurrency(475 - 475*Discount/100,0) 
    else
       PurpleAuctionPrice = formatcurrency(475,0) 
 end if 

PurpleDutchAuctions = Request.Form("PurpleDutchAuctions")
if len(PurpleDutchAuctions) > 0 then
    TotalPurpleDutchAuctionsOrder = PurpleAuctionPrice * PurpleDutchAuctions
    TotalOrder = TotalOrder + PurpleAuctionPrice * PurpleDutchAuctions
 else
 PurpleDutchAuctions = 0
end if
...
if discount > 0 then 
    BlueAuctionPrice = formatcurrency(355 - 355*Discount/100,0) 
    else
        BlueAuctionPrice = formatcurrency(355,0) 
 end if 
BlueDutchAuctions = Request.Form("BlueDutchAuctions")
if len(BlueDutchAuctions) > 0 then
    TotalBlueDutchAuctionsOrder = BlueAuctionPrice * BlueDutchAuctions
    TotalOrder = TotalOrder + BlueAuctionPrice * BlueDutchAuctions
     else
BlueDutchAuctions = 0
end if

if discount > 0 then 
   RedAuctionPrice = formatcurrency(198 - 198*Discount/100,0) 
   else
    RedAuctionPrice = formatcurrency(198,0) 
 end if 
RedDutchAuctions = Request.Form("RedDutchAuctions")
if len(RedDutchAuctions) > 0 then
    TotalRedDutchAuctionsOrder = RedAuctionPrice * RedDutchAuctions
    TotalOrder = TotalOrder + RedAuctionPrice * RedDutchAuctions
else
RedDutchAuctions = 0
end if

if discount > 0 then 
   YellowAuctionPrice = 32
else
   YellowAuctionPrice = 98
 end if 
 
YellowDutchAuctions = Request.Form("YellowDutchAuctions")
if len(YellowDutchAuctions) > 0 then
    TotalYellowDutchAuctionsOrder = YellowAuctionPrice * YellowDutchAuctions
    TotalOrder = TotalOrder + YellowAuctionPrice * YellowDutchAuctions
else
YellowDutchAuctions = 0
end if
...
<form  name=UpdateOrderForm method="post" action="MembersAdvertisingAddFrameSignup.asp?Updatetable=True">
<table cellpadding= "0" cellspaccing = "0" width = "920">
<tr bgcolor = "#EEDD99">
...
<a class="tooltip" href="#"><b>Yearly Membership:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Alpaca Infinity Yearly Membership</em>List unlimited alpacas and products for sale as well as have your cusomizable farm pages.<br /></span>

</td>
<td class = "body2" align = "right">
<% if discount > 0 then %>
<strike>$85/year</strike>
<% else %>
<b>$85/year</b>
<% end if %>
</td>
...
<td class = "body" align = "left" colspan = "13" bgcolor = "grey">

</td>
<td class = "body2" align = "right">
<% = formatcurrency(TotalMembershipOrder, 2)%>
</td>
</tr>
<tr>
<td class = "body" align = "left">

<a class="tooltip" href="#"><b>Logo Ad (180px X 50px):</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Logo Ad</em>Logo Ads are<b>180 pixels wide by 50 pixels high</b>. They appears on major pages throughout Alpaca Infinity and are displayed in a <b>pool of 60 ads</b>.</span></a>
</td>
<td class = "body2" align = "right">
<% if discount > 0 then %>
<strike>$50/mo</strike>
...
Query =  "Select AdType from Ads  where not(PeopleID = 1016) and adWebsite = 'Alpaca Infinity' and AdMonth= '" &  Month(currentdate)  & "' and AdYear= '" & Year(currentdate)  & "' and AdType='Logo Ad' and AdPaidFor=True" 
...
<a class="tooltip" href="#"><b>Banner Ad (400px X 100px):</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Banner Ad</em>Banner Ads are <b>400 pixels wide by 100 pixels high</b>. They appears on major pages throughout Alpaca Infinity and are displayed in a <b>pool of only 50 ads</b>.</span></a>
</td>
<td class = "body2" align = "right">
<% if discount > 0 then %>
<strike>$75/mo</strike>
...
Set rs = Server.CreateObject("ADODB.Recordset")
Query =  "Select AdType from Ads  where not(PeopleID = 1016) and adWebsite = 'Alpaca Infinity' and AdMonth= '" &  Month(currentdate)  & "' and AdYear= '" & Year(currentdate)  & "' and AdType='Banner Ad' and AdPaidFor=True" 
	rs.Open Query, conn, 3, 3
	while not rs.eof 
...
<a class="tooltip" href="#"><b>Skyscraper Ad (150px X 250px):</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>skyscraper Ad</em>Grab attention with Skyscraper Ads. these ads are <b>150 pixels wide by 250 pixels high</b> and they appear prominently on major pages throughout Alpaca Infinity and are displayed in a <b>pool of only 35 ads!</b></span></a>
</td>
<td class = "body2" align = "right">
<% if discount > 0 then %>
<strike>$95/mo</strike>
...
Set rs = Server.CreateObject("ADODB.Recordset")
Query =  "Select AdType from Ads  where not(PeopleID = 1016) and adWebsite = 'Alpaca Infinity' and AdMonth= '" &  Month(currentdate)  & "' and AdYear= '" & Year(currentdate)  & "' and AdType='Skyscraper Ad' and AdPaidFor=True" 
	rs.Open Query, conn, 3, 3
	while not rs.eof 
	...
	wend
%>
<% if SkyscraperAdsExisting > 35 then %>
...
<a class="tooltip" href="#"><b>Mega Ad Combo!:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Mega Ad Combo</em>The Mega Ad Combo is one kick ass marketing bonanza! It includes a <b>full-screen background ad image (1900 pixels by 900 pixel)</b> and will be seen on every page except farm pages that set their own background image. <b>Plus a 400 pixel x 75 pixel header ad.</b> And there is only one of these per month <b>(Pool of 1)</b>!</span></a>
...
Set rs = Server.CreateObject("ADODB.Recordset")
Query =  "Select AdType from Ads  where adWebsite = 'Alpaca Infinity' and AdMonth= '" &  Month(currentdate)  & "' and AdYear= '" & Year(currentdate)  & "' and AdType='Mega Ad Combo' and AdPaidFor=True" 
	rs.Open Query, conn, 3, 3
	while not rs.eof 
...
<a class="tooltip" href="#"><b>Professional Account Setup:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Professional Account Setup:</em>A one-time fee to setup your account for you. We will load up to 10 alpacas from any other internet site to your new Alpaca Infinity site, help you set up your graphics (logo, backgrounds, etc.), and you get a free logo ad that will run for 3 months. <br /></span>
...
<a class="tooltip" href="#"><b>Mass Email:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Mass Emails</em>Mass email marketing is a reliable way for you to connect with potential customers. Our Mass Email service include:
	   <ul><li>Email Creation.</li>
	  <li>Sending to our list of <b>over 7,000 alpaca email addresses</b>.</li> 
</ul><br /></span>
...
<tr >
<% if discount > 0 then %>
<td  colspan = "17" class = "body" bgcolor = "#EEDD99">
<% else %>
<td  colspan = "16" class = "body" bgcolor = "#EEDD99">
<% End if %>
<b>Dutch Auctions</b>
</td></tr>
<tr>
<td class = "body" align = "left">
<a class="tooltip" href="#"><b>Purple Level:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Purple level Dutch Auction</em><ul>
...
<% if discount > 0 then %>
<td class = "body2" align = "right">
<b><%=formatcurrency(475 - 475*Discount/100,0) %>&nbsp;&nbsp;</b>
</td>
<% end if %>
<td class = "body" align = "left" colspan = "13">Quantity:
<select size="1" name="PurpleDutchAuctions" onChange="UpdateOrderForm.submit()">
	<% if len(PurpleDutchAuctions) > 0 then %>
		<option value="<%=PurpleDutchAuctions %>" selected><%=PurpleDutchAuctions %></option>
		<option value="0" >0</option>
	<% else %>
...
<td class = "body2" align = "right">
<% = formatcurrency(TotalPurpleDutchAuctionsOrder, 2)%>
</td>
</tr>

<tr>
<td class = "body" align = "left">
<a class="tooltip" href="#"><b>Blue Level:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Blue level Dutch Auction</em><ul>
...
<% if discount > 0 then %>
<td class = "body2" align = "right">
<b><%=formatcurrency(355 - 355 *Discount/100,0) %>&nbsp;&nbsp;</b>
</td>
<% end if %>
<td class = "body" align = "left" colspan = "13">Quantity:
<select size="1" name="BlueDutchAuctions" onChange="UpdateOrderForm.submit()">
	<% if len(BlueDutchAuctions) > 0 then %>
		<option value="<%=BlueDutchAuctions %>" selected><%=BlueDutchAuctions %></option>
		<option value="0" >0</option>
	<% else %>
		<option value="" selected>0</option>
	<% end if  %>
...
<td class = "body2" align = "right">
<% = formatcurrency(TotalBlueDutchAuctionsOrder, 2)%>
</td>
</tr>
...
<tr>
<td class = "body" align = "left">
<a class="tooltip" href="#"><b>Red Level:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Red level Dutch Auction</em><ul>
       <li>1 alpaca</li>
        <li>Preferred position on auction listings.</li>
        <li>Inclusion in weekly auction emails.</li>
        <li>Skyscraper ads (6 weeks).</li>
...
<% if discount > 0 then %>
<td class = "body2" align = "right">
<b><%=formatcurrency(198 - 198 *Discount/100,0) %>&nbsp;&nbsp;</b>
</td>
<% end if %>
<td class = "body" align = "left" colspan = "13">Quantity:
<select size="1" name="RedDutchAuctions" onChange="UpdateOrderForm.submit()">
	<% if len(RedDutchAuctions) > 0 then %>
		<option value="<%=RedDutchAuctions %>" selected><%=RedDutchAuctions %></option>
		<option value="0" >0</option>
	<% else %>
		<option value="" selected>0</option>
	<% end if  %>
...
<td class = "body2" align = "right">
<% = formatcurrency(TotalRedDutchAuctionsOrder, 2)%>
</td>
</tr>
...
<tr>
<td class = "body" align = "left">
<a class="tooltip" href="#"><b>Yellow Level:</b><span class="custom info"><img src="/images/logoTip.png" alt="Alpaca Infinity Screen Tip" height="48" width="48" /><em>Yellow level Dutch Auction</em> <ul>
           <li>1 alpaca.</li>
           <li>Website auction listing.</li>
           <li>Logo ads (6 weeks).</li>
         </ul><br /></span>
...
<% end if %>
<td class = "body" align = "left" colspan = "13">Quantity:
<select size="1" name="YellowDutchAuctions" onChange="UpdateOrderForm.submit()">
	<% if len(YellowDutchAuctions) > 0 then %>
...
<% else %>
<td colspan = "15" align = "right" class = "body">
<%end if %>Amount Available in <i>Advertising Account*</i>
:
<%
...
<b><% = formatcurrency(CurrentAccount)%></b></td>
</td>
<td></td>
</tr>
<% end if %>
...
<% else %>
<td colspan = "15" align = "right" class = "body">
<%end if %><% if CurrentAccount > 0 then %>
...
<tr>
<% if discount > 0 then %>
<td class = "body2" colspan = "16" align = "right" > 
<% else %>
...
<input type="image" src="images/paynow.jpg" border="0" name="submit" >
 ...
 <img src = "/uploads/ProfessionalSetupBannerAd.jpg"  align = 'left' width = "403" height = "102"/>
 </td>
 <td>
  <img src = "/uploads/AuctionBannerAd.jpg"  align = 'left' width = "403" height = "102"/>
...
<% if CurrentAccount > 0 then %>
<b>* Advertising Account:</b> Sometimes you may receive credit (as a result of an Alpaca Infinity promotion) that can be used for advertising with Alpaca Infinity. This credit goes into your advertising account. <b>Funds in your Advertising Account may only be used for advertising with Alpaca Infinity and may not be withdrawn. Advertising Account funds are non transferable.</b><br /><br />
