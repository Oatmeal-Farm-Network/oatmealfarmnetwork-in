...
response.write("AdvertisingLevelID = " & AdvertisingLevelID &"<br/>")
...
response.write("SpecialRequests = " & SpecialRequests & "<br/>")
if len(AdvertisingQTY)> 0 then
else
AdvertisingQTY = 0
end if
...
response.write("AdvertisingAddHandle People Query = " & Query & "<br/>")
...
'****************************************************************************************************
'  INSERT THE Advertising INFO
'****************************************************************************************************
Query =  "INSERT INTO Advertising (BusinessID, AddressID,  AdvertisingQTY, AdvertisinglevelID, "

if len(AdvertisingPaidAmount) > 0 then
	Query =  Query & " AdvertisingPaidAmount , " 
end if 
if len(AdvertisingPaidAmountMonth) > 0 then
	Query =  Query & " AdvertisingPaidAmountMonth , " 
end if 
if len(AdvertisingPaidAmountDay) > 0 then
	Query =  Query & " AdvertisingPaidAmountDay , " 
end if 
if len(AdvertisingPaidAmountYear) > 0 then
	Query =  Query & "  AdvertisingPaidAmountYear , " 
end if 
Query =  Query & " SpecialRequests,  EventID )" 
Query =  Query & " Values (" & BusinessID & ", "
Query =  Query & " " & BusinessAddressID & ", " 
Query =  Query & " " & AdvertisingQTY & ", " 
 
Query =  Query & " " & AdvertisingLevelID & ","
If len(AdvertisingPaidAmount) > 0 then
	Query =  Query & " " & AdvertisingPaidAmount & "," 
end if
response.write("AdvertisingPaidAmount = " & AdvertisingPaidAmount & "<br/>")
...
response.write("Line 315 Query = " & Query & "<br/>")
Conn.Execute(Query) 

response.redirect("Advertisingadd.asp?EventID=" & EventID & "&message=Your Advertising has been added.")
