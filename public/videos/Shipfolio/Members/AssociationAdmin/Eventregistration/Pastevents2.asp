  ...
  wend
	...
	rs.close
		...
		If Not view  = "All" And Len(view) > 1 then
			  ...
			  sql = "select * from Events, EventsCategories where Events.EventsCatID = EventsCategories.EventsCatID and EventsCategories.CategoryName = '" & view & "' order by cint(EventsDateYear), cint(EventsDateMonth), cint(EventsDateDay) "
			Else
				
			 sql = "select * from Events, EventsCategories where Events.EventsCatID = EventsCategories.EventsCatID  order by cint(EventsDateYear), cint(EventsDateMonth), cint(EventsDateDay)"
			 End If 
