   ...
   WYSIWYG.attach("A<%=rowcount%>");
	...
	<%	ClassInfoTitle = rs("ClassInfoTitle")
		...
		<tr >
  			...
  			<table>
  			<tr>
  				<td class = "body2"  align = "right" valign = "top"><small>Class Titless: *</small></td>
  				<td>
   		...
   		<tr>
  			...
  			<td>  
  				...
  				<select size="1" name="InstructorID(<%=rowcount%>)">
  	
  		<% if len(instructorPeopleID) > 1 then
...
<% 
	...
	<option value="<%=instructorPeopleID %>">None</option>
...
<% 'response.write("rowcount = " & rowcount & "ClassInfoDescription" & ClassInfoDescription & "<br>")%>
 <td class = "body2"><small>Description:</small><br><TEXTAREA NAME="ClassInfoDescription(<%=rowcount%>)" cols="70" rows="11" wrap="file" ID="<%=rowcount%>" ><%=ClassInfoDescription%></textarea><br>
 	<small>Homework:</small><br><TEXTAREA NAME="ClassHomework(<%=rowcount%>)" cols="70" rows="7" wrap="file" ID="A<%=rowcount%>"><%=ClassHomework%></textarea>
 </td>
</tr>
</table>
