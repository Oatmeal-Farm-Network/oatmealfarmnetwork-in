...
'mobile_browser = 1
  CookieScreenWidth=Request.Cookies("ScreenWidth")
Error=Request.querystring("Error")
  ScreenWidth=Request.querystring("ScreenWidth")
  RealScreenWidth =  CookieScreenWidth
  ' response.write("ScreenWidth=" & ScreenWidth ) 
 'response.write("CookieScreenWidthA=" & CookieScreenWidth )

 if len(CookieScreenWidth) > 1 and len(ScreenWidth) > 1 then
  if cint(CookieScreenWidth) > cint(ScreenWidth) then
  ScreenWidth = CookieScreenWidth
  end if
end if
'response.write("ScreenWidthB=" & ScreenWidth )
...
<link rel="stylesheet" type="text/css" href="/associationadmin/associationMobilestyle.css" />
<% else %>
<link rel="stylesheet" type="text/css" href="/associationadmin/associationstyle.css" />
<% end if 
end if %>