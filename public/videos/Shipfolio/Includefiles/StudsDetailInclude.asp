...
'response.write("Sortbysearch=" & Sortbysearch )
...
sql= sql & " WHERE animals.speciesID = " & speciesID & " and animals.PeopleID = People.PeopleID  and  animals.BreedID = SpeciesBreedLookupTable.BreedLookupID and People.AddressID = address.AddressID And pricing.id=Animals.id and Ancestrypercents.id=Animals.id And Photos.id=Animals.id  and Colors.id=Animals.id and subscriptionlevel > 0 and PublishStud = 1 and people.country_id = " & country_id & " and  sold = 0 " & Categorysearch & Breedsearch & Colorsearch &  OBOSearch & currentminStudFeeSearch & currentmaxStudFeeSearch &  QAncestry & CategorySearch & Statesearch  
...
<tr><td class = 'body'><%= adstart %> - <%= adend %> Listings of <%=count %>&nbsp;<br /> </td>
<% if cint(Currentpagenumber) < cint(totalpages) and cint(totalpages) > 1  then %>
<td class = "Pagebox" width = "12"><a href = "studs.asp?pagenumber=<%=pagenumber + 1 %>&<%=searchvariables %>" class = "NumLinks"><b>></b></a></td>
<% end if %>
<td width = 20></td>
</tr></table>
<% else %>
<div align = Right><%=count %> Result<% if count > 0 then %>s<% end if %><br /></div>
<% end if %>
...
sql= sql & " WHERE animals.speciesID = " & speciesID & " and animals.PeopleID = People.PeopleID  and People.AddressID = address.AddressID And pricing.id=Animals.id and Ancestrypercents.id=Animals.id And Photos.id=Animals.id  and Colors.id=Animals.id and subscriptionlevel > 0 and PublishStud = 1 and people.country_id = " & country_id & " and  sold = 0 " & Categorysearch & Breedsearch & Colorsearch &  OBOSearch & currentminStudFeeSearch & currentmaxStudFeeSearch &  QAncestry & QPercentAccoyo & CategorySearch & Statesearch  
sql= sql & " ) "
sql= sql & " SELECT * "
...
SpeciesCategoryID = rs("Category")

sql2 = "select SpeciesCategory, SpeciesCategoryPlural from SpeciesCategory where SpeciesCategoryID=" & SpeciesCategoryID 
...
<div class="container-fluid" align = "center" >
    ...
    <div class = "row nowrap" align = "center" >
        <div class = "col-4 body box box2" style="min-width:250px; max-width: 250px;">
       <a href = "/Animals/Details.asp?ID=<%=AnimalID%>&DetailType=<%=DetailType%>&CurrentPeopleID=<%=CurrentpeopleID %>" >
           <img src = "<%= PhotoID %>" class="img" alt = "Image" style="border:1px solid black;" align=center width="200px"></a>
        </div>
        <div class = "col-3 body" border = 0  style="min-height:220px; max-height: 220px;">  
        <br />
<% Price = trim(rs("price"))
...
Stud Fee: <b><%=(FormatCurrency(Studfee,0))%> </b><br><br />
<% else %>
<b>Contact Us For Stud Fee</b><br>
<% End If %>
<br />
<b>For Sale By</b></br/>
...
<% end if %>
   <form NAME="Details" ACTION="/Animals/Details.asp?ID=<%=AnimalID%>&viewstud=True" METHOD="POST">	
      <input type = "submit" value="View" class = "regsubmit2" />&nbsp; &nbsp; &nbsp;
   </form>
       </div>
    </div>
</div>
...
<% if cint(pagenumber) = cint(currentpagenumber) then %>
 ...
 <a href = "studs.asp?pagenumber=<%=pagenumber %>&<%=searchvariables %>" class = "NumLinksCurrent"><b><center><%=pagenumber %></center></b></a></td>
 <% else %>
<td class = "Pagebox" width = "18">
<a href = "studs.asp?pagenumber=<%=pagenumber %>&<%=searchvariables %>" class = "NumLinks"><b><center><%=pagenumber %></center></b></a></td>
<% end if %>
...
<% if cint(pagenumber) = cint(currentpagenumber) then %>
 ...
 <a href = "studs.asp?pagenumber=<%=pagenumber %>&<%=searchvariables %>" class = "NumLinksCurrent"><b><%=pagenumber %></b></a></td>
 <% else %>
 <% if pagenumber> 0 then %>
<td class = "Pagebox" width = "12">
<a href = "studs.asp?pagenumber=<%=pagenumber %>&<%=searchvariables %>" class = "NumLinks"><b><%=pagenumber %></b></a></td>
<% end if %>
<% end if %>
